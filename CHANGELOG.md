# PackageManager 变更日志

本文件基于仓库 `gitlog.md` 的提交历史按版本号归档,概述每个版本的主要改动与新增功能。

## 4.3.1.4 — 2026-09-08

- 看板与详情终态观感：
  - 看板已完成与已关闭列的卡片整体灰化：内容降不透明并轻去色，标题加删除线，对齐网页端观感。
  - 详情页终态标题改灰色加删除线，状态下拉切换后删除线实时跟随，打开即知完成与否。

- 工作项详情标题编辑：
  - 标题改单击就地编辑：原元素直接编辑零位移零字体变化，光标落在点击位置；编辑中无删除线，保存或取消后恢复。
  - 标题字重对齐网页常规样式，编号不再被挤压换行。

- 看板统计图表：
  - 统计表格下方新增图表区，渲染九种统计图：成员负载堆叠柱、完成率结构横条、负载帕累托、状态与优先级环形图、迭代完成率仪表盘、成员对比雷达、优先级分组柱、负载产出气泡散点。
  - 图表底部自动生成通俗结论：负载与完成率排名、集中度与单点风险、高优分布、成员对比与产出标杆。
  - 修复图表模板脚本语法错误，该错误曾致整个图表区不渲染；结论框对齐与图例顺序统一。
  - 查询期间隐藏图表区，遮罩淡出后恢复显示，消除闪烁。

- 性能优化：
  - 看板可达集预热改为按方案一次拉全量流转，替代逐状态串行探测，打开期不再阻塞数秒。
  - 详情附件元信息改并发获取，多图评论打开不再逐图串行等待。
  - 代码差异新旧文本并行取回并统一换行，窗口就绪即后台预取旧文本，点开文件近零等待；测宽去抖消除大文件渲染卡顿。
  - 拉取冲突检测仅扫未合并文件，干净仓库不再全树扫描；设置读取按文件时间戳缓存、FTP 凭证解析移入后台，周期任务不再抢占界面渲染。
  - 新增看板性能自动化测试入口，三轮测速按阈值判定并输出报告。
  - 详情页超长标题自动换行，不再挤压 AI 按钮。

- 看板权限与去浏览器化（受限功能）：
  - 经典视图兜底入口改与 AI 拆解/AI 实现同一权限门控，非授权用户不再显示该入口。
  - 看板网页视图去浏览器化：右键菜单、开发者工具与浏览器快捷键全部关闭。

- 提交参考：
- 0db410764ed9bc7a23a005e57ce1dc4d3f918ac3 — feat(PackageManager): 经典视图按权限隐藏并看板去浏览器化
- 0c4281bd86ab7f0ae097398085e42563d10812ac — feat(PackageManager): 详情终态标题删除线并改单击编辑
- 2b7ce4982ae224d92759638621eb234fdfacbdc7 — style(PackageManager): 看板终态卡片灰化对齐网页观感
- d652887073aa0bed094f51a3ee50130f3d16d69d — feat(PackageManager): 统计图自动生成结论解读并修复不渲染
- 9aa6063228e9d91383312bd9131cc6f9c159f4f3 — feat(PackageManager): 看板统计新增九种迭代统计图
- 6bf5e3ed9a89506797c0af6053a78ac3538e7d16 — perf(PackageManager): 看板详情与代码差异打开提速，周期任务不再卡界面

## 4.3.1.3 — 2026-08-26

- 看板拖拽落库与预校验：
  - 拖拽改状态改为后台队列写入：支持连续拖拽与同卡改向，写入期间卡片不被刷新弹回；关窗等待落库，超时征询是否丢弃。
  - 拖起卡片按状态流转规则预校验目标列：不可达列显示禁止光标且松手无效，不再事后报错回弹；短列与最长列等高，滚动深处任意列均可直接拖入。
  - 重复执行的写入按源状态指纹静默丢弃不误报；写入失败改为标题栏徽标短时提示，不打断连续拖拽。
  - 共享详情窗增加浏览器进程监护，进程故障自动销毁重建；严重程度由选项 ID 统一翻译为文本并补充「建议」级别颜色。

- 看板可达集与拖拽预亮：
  - 看板打开期后台预热全部工作项可达集，完成后补推整板，任意卡拖拽均为就绪数据。
  - 拖放落卡瞬间即应答目标状态可达集，单卡直达更新不走整板渲染，二次拖拽即拿最新集合。
  - 拖起卡片即预亮全部可达列（虚线弱高亮），无需悬停即可判断能否放置；预热未完成就开拖时按宿主应答实时收窄预亮。
  - 可达集解析统一加互斥保护，预热、应答与拖拽解析并发访问不再损坏共享缓存；卡片签名纳入更新时间，工作项变更后整板推送能正确刷新。

- 工作项详情页就地编辑：
  - 负责人改为成员下拉，选择即写入并同步显示，失败自动还原；标题双击就地编辑，回车或失焦保存、Esc 取消，失败还原原文。
  - 描述支持就地编辑与插图：粘贴、拖拽或选文件插入图片；保存前剥除图片地址中的访问令牌，令牌仅本地显示不写入服务端。
  - 写入补全成败日志与请求预览，便于排查指派与描述保存失败。

- 详情插图内联直存与上限：
  - 插图改以 data:URL 内联直存描述，自包含永不过期、双端可渲染，不再挂入工作项附件列表，保存不再被图片上传阻塞。
  - 插图新增总量拦截：累计超限即提示已用与本张大小，引导压缩后再试；插入图片后光标折叠到图片之后，消除选中高亮残留。

- 详情窗僵尸单例自愈：
  - 共享详情窗防宿主级联关闭并自愈：隐藏时解除宿主归属，被关闭即清缓存，打开失败销毁重建重试，不再依赖重启应用。

- 提交工作项任务类型与会话续期：
  - 提交工作项新增「任务」类型：创建前查项目内真实任务类型 ID，规避 task 短名不被接口接受的问题。
  - 任务无示意图字段，图片改按网页编辑器同构标签追加到描述，避免预览异常或描述被抹空；示意图区标题随类型切换提示图片去向。
  - 新增 PingCode 会话无感续期：轻量探测顺带续期落盘，失效再走隐藏 WebView2 静默续期；
  - 页面加载后台预续期，上传失败自动续期并重试一次，过期不再需要手动登录。

- 看板统计迭代记忆：
  - 手选迭代即持久化，与工作日报存储独立、互不干扰，重开按同项目恢复。
  - 默认迭代与工作日报同源推荐：上次手选优先，其次今天所在区间、结束最近的未来迭代；过期自动切换并在结果区提示。
  - 迭代列表排序对齐工作日报，最近开始在前。

- 提交工作项与看板渲染修复：
  - 提交工作项页登录按钮随登录态探测结果校准：有效显示「切换账号」、失效显示「登录PingCode」，一眼看出是否需重新登录。
  - 看板被详情窗完全遮挡后关闭详情窗不再整屏闪烁；代价是被遮挡期间不省渲染资源。

- 提交参考：
- c6c3b433908a2cce4e2d38c8eec10bbd8feb64a1 — feat(PackageManager): 看板统计记忆迭代选择并修复遮挡闪烁
- b0292b55c1f8929b35557ba5ae1a3c8efdfb6858 — feat(PackageManager): 看板拖起即预亮可达列
- 4a28f144a455367eedb22e4bf87b968d537ea972 — feat(PackageManager): 提交工作项支持任务类型，PingCode 会话无感续期
- d0a36b495f3db9d9ad39a40c9c37bedfbd048d38 — fix(PackageManager): 详情插图加总量上限拦截并修正插入后光标
- 6a7cded271583ffaf7d5aff860eaa3c3242c74b7 — fix(PackageManager): 详情描述插图改为内联直存，摆脱附件地址依赖
- df71cc8763259a76c04956254fd213fa4be75195 — feat(PackageManager): 工作项详情页支持就地编辑负责人、标题与描述
- be1187a7605bd75a4f0d6f7a2806b0bc292d0c5d — feat(PackageManager): 看板可达集预热与拖放即时应答，详情窗僵尸单例自愈
- 49f058a9e3d5c627f0c748e9772fb02e6e8a7d50 — fix(PackageManager): 看板可达缓存键补项目维度并写入后预热
- 6ee1d03498d4b8ad4b0eb8edb890ee623fd7632c — feat(PackageManager): 看板拖拽按流转规则预校验目标列
- 6ad158124fc5352b5ee1a338d9a7ce24413190d8 — fix(PackageManager): 看板拖拽写入防重复执行并降日志级别
- 672c3522358b633d509cb082c5ab59dc68d94c1d — fix(PackageManager): 看板短列拉伸等高优化拖放目标
- 8c53b73479f60102623ac23b5a5801f5e27f0aa4 — fix(PackageManager): 看板拖拽改为后台落库并加固详情窗

## 4.3.1.2 — 2026-08-24

- 更新窗口修复：
  - 版本徽章改用固定高度与半高圆角并垂直居中文字，呈现标准胶囊形且两徽章高度一致。

- 迭代看板与工作项详情重做：
  - 看板改由内嵌网页渲染：整页滚动、列头常驻、卡片拖拽换状态即时生效，保留经典视图兜底入口。
  - 详情界面重做：AI 入口并入网页头部，子工作项以统计条与表格呈现完成进度和故事点；所属产品等自定义字段翻译为显示文本。

- 看板与详情性能优化：
  - 启动预热共享环境与详情窗，打开即用，不再有数十秒阻塞；详情窗共享复用，切换仅重新导航。
  - 看板轮询改为增量渲染不再整屏闪烁，列内排序稳定；卡片悬停预取详情，点击即开。
  - 切换工作项旧内容零泄漏，加载遮罩带转圈动画。

- 代码工作区刷新提速：
  - 子仓库采集并行化并加并发限流，目录发现单趟遍历加缓存，刷新不再有进程风暴。
  - 状态应用改为差分更新，轮询不再整表重绑；等宽测量按字符缓存，排版更快。

- 密语离线投递与同事列表：
  - 对方离线或直连失败时自动经信箱投递，对方上线自动接收并回执，消息不再丢失；已收到的直连消息按消息号去重。
  - 消息端到端加密传输，本地会话加密落盘，重启后恢复未读与投递状态；信箱信封超 24 小时自动过期清理。
  - 同事列表合并在线设备与离线联系人，徽标区分在线/离线与计数；信箱轮询频率接入设置（默认 60 秒，可关闭）。
  - 发布工具的更新清理保留服务器上的密语信箱目录。

- 密语已读与界面重构：
  - 已读改为视口触发：气泡进入视口约 1 秒才标记已读，打开窗口自动定位第一条未读；失焦或关窗即焚毁已读消息。
  - 聊天界面重做：气泡内嵌引信进度条、未读高亮与销毁占位，头部改为头像布局；自毁时长接入设置（5/10/30 秒）。

- 提交参考：
- 937ce6658dbf7e872a185405e6029a398ad039d8 — perf(PackageManager): 详情下拉数据预取提前并修复转圈动画
- 73ed0b28a10e0bbcd46f56beec83cf56c96e0937 — perf(PackageManager): 详情切换消除旧内容闪现并缓存低频数据
- fca4355550d7a3329e46c38540c59e7e81d0076f — perf(PackageManager): 看板与详情打开提速并消除刷新闪烁
- 62cc27353b752b833411b603eccae871e3e88d7b — feat(PackageManager): 看板改用网页渲染并重设计工作项详情
- 94edc10212070565ac5e633237e37d5e03588df0 — fix(PackageManager): 工作项所属产品改为显示文本
- 9b2f9d342f1b4886c350d9b9148adbe1310c0d84 — perf(PackageManager): 代码工作区刷新提速并消除轮询重绘
- dd7dafb2b8954e9d495d48440241917a557547d5 — perf(PackageManager): 优化密语信箱拉取并发与超时并同步产物
- a719df0c3ba831dc570729bdcba82ba8ae9bd14d — fix(PackageManager): 修复密语离线投递可靠性并归档 4.3.1.2
- c96defc68069d7c6dd79ea8a337bbb7eb94d1014 — feat(PackageManager、FtpPublisher): 密语支持离线信箱投递并统一同事列表
- 6bf31ac40b2c64111d6e432e4389daa203c6611a — feat(PackageManager): 密语已读判定改为视口触发并支持失焦即焚
- 338c7b87a8adcb54cb91aa7621a00f2da3b1f9e4 — fix(PackageManager): 修正更新窗口版本徽章样式并同步 MftScanner 产物

## 4.3.1.1 — 2026-08-20

- 自动更新体验重构：
  - 新版本提示从系统消息框改为独立更新窗口，展示当前→最新版本徽章与按版本分组的更新点时间线（新增/修复/优化标签）。
  - 更新窗口内直接下载并显示进度、速度与预计剩余时间，失败后原地重试，不再弹第二个消息框。
  - 新增「稍后提醒」：转入通知中心保留更新入口；「跳过此版本」记住选择，更高版本恢复提醒。

- 提交参考：
- 998ea4c0d5e7af3b98b6f3ed9942fd03b1d5a1cf — feat(PackageManager): 自动更新改用自定义更新窗口，支持更新点时间线、下载进度与跳过版本

## 4.3.1.0 — 2026-08-20

- 本地路径配置收敛：
  - 版本路径配置归一化为「产品默认 + 版本例外」，加载存量数据与保存设置时自动收敛。
  - 与产品默认相同的版本级路径改为继承，FTP 新出现的版本自动沿用默认路径且不覆盖已有设置。
  - 设置页保存时按全组众数确定产品默认路径，与设置窗口行为保持一致。
  - 分组表格组头宽度改为 XAML 绑定并开启分组虚拟化，移除代码后置遍历视觉树设置宽度。

- PingCode看板修复：
  - 迭代工作项查询删除 iteration_id 兜底参数，避免开放 API 静默忽略后误拉全库工作项；
  - sprint_id 查询为空即判定该迭代无工作项，直接结束分页。

- AI 提交技能分发：
  - AI 提交前的技能同步从单一 skill 泛化为多技能框架，辅助技能按清单同步到用户级 .claude/.codex 目录。
  - 新增远程 PUA 规范加载器 load_pua_skill 作为首个辅助技能，随 EXE 内嵌并在提交前同步到用户机器。

- 工作日报迭代选择（受限功能）：
  - 日报页新增迭代下拉框，展示项目未完成迭代与起止日期，生成日报时按所选迭代拉取工作项并在状态栏显示数据源。
  - 默认迭代智能推荐：上次手选优先，其次今天所在区间、结束最近的未来迭代、最近开始，替代原先的名称排序。
  - 手选迭代保存到本地，下次打开自动恢复；上次选择已失效时自动切换并提示。
  - 迭代实体与接口解析补齐结束时间，支撑区间判断与展示。

- 提交参考：
- 425695383c2036ded422ec0dacdf7e257949c85f — feat(PackageManager): 工作日报支持迭代选择与默认迭代智能推荐
- 2e52ad29fe3c3fbbfa042b8de7990b6b09ba2a9f — feat(PackageManager): AI 提交技能同步支持辅助技能分发，内嵌 load_pua_skill 加载器
- c69ee84af3844c1733e553be4363f6de50274e25 — feat(PackageManager): 本地路径配置收敛为产品默认加版本例外，新版本自动沿用默认路径
- f9d89d3afa61d240b6f2198ebb5756f58da7c7c3 — fix(PingCode): 迭代工作项查询删除 iteration_id 兜底，避免误拉全库工作项目

## 4.3.0.0 — 2026-08-14

- 「提交工作项」页：
  - 新增「提交工作项」页，粘贴群聊自动拆出标题、描述与示意图，一键提交到当前项目进行中的迭代。
  - 创建按「建项→传图→写示意图」三步执行，取不到永久图址时降级为附件关联并提示。
  - 粘贴优先读取文件原始字节以保留 GIF 动画，视频自动作为附件上传，示意图仅支持图片。
  - 页面改为左描述、右素材的双列布局，标题可编辑且不再被自动提取覆盖。
  - 新增 PingCode 登录窗口与 Cookie 持久化，示意图走内部 atlas 永久 URL，附件扩展到任意文件。
  - 子工作项列表按编号尾号重排，对齐网页端展示顺序。

- MiMo 用量统计：
  - 新增 MiMo 用量功能页，通过内嵌浏览器登录并持久化 Cookie，支持会话自动恢复。
  - 拉取当月每日 Token 用量与套餐信息，按日期和模型维度汇总统计，支持按月切换。
  - 套餐用量拆为已用、总量、剩余三栏，剩余不足两成时标红，数字格式化抽到公共工具类。
  - 新增切换账号，清空登录态与缓存并登出服务端后回到登录页。
  - 登录改用独立窗口并在成功后落盘 Cookie，修复登录态过期与切换账号无法重建缓存的问题。

- 索引宿主自愈与搜索性能：
  - 共享层新增宿主健康探活（进程存活 + 心跳鲜度）与强杀重启自愈，索引宿主死亡或假死时自动恢复。
  - 宿主侧新增搜索执行看门狗，App 新增后台看门狗周期探活并节流自愈，补上引擎假死盲区。
  - 搜索与标记删除命令新增响应超时预算，把死/卡宿主的无限等待转为可自愈的快速失败。
  - 新增 prefix4 前缀索引加速 native v2 前缀查询，运行时快照升级并保持旧版兼容加载。
  - overlay 压缩决策重构为三级决策（硬阈值、软阈值、时长老化），快照保存支持锁外构建与强制重试。
  - 单卷 USN 失效时后台重建该卷快照并热切换，保留其他卷和 overlay 状态继续服务。
  - 修复 overlay 快照基准失配后的重试恢复，以及修复窗口内删除变更被旧快照恢复丢失的问题。

- AI 工作区与 CLI 体验：
  - 代码工作区新增 MSBuild 构建入口，支持选择项目、配置和 Restore 策略并记忆上次选择。
  - 拉取逻辑重构为 PullStep 步骤模型，支持实时进度显示与逐项结果汇总。
  - PingCode 工作项详情新增 AI 拆解入口，拆解前并发获取子工作项并去重，子项描述结构化输出（受限功能，暂不开放）。
  - 内网原型资源预下载并写入 Prompt；复制 Prompt 同样走完整资源准备链路。
  - AI prompt 改为文件化管理并自动清理过期文件，工作项图片按工作项隔离保存。
  - AI CLI 新增权限模式与终端启动模式配置，启动状态同步显示当前模式。
  - AI 提交流程状态目录隔离，避免并发提交互相覆盖；启动前预热并串行仓库操作。
  - CodeGraph 探索协议注入与 MCP 配置自动同步；全局行为规则改为项目级记忆管理，并新增图片理解兜底规则。

- 工作日报（受限功能，暂不开放）：
  - 新增工作日报页面，按日期采集 Git/SVN 提交并生成可编辑日报，提供复制到剪贴板流程。
  - 合并 Git、SVN 和 PingCode 当天完成任务，支持混合仓库扫描与作者过滤。
  - 明日计划对齐看板默认项目迭代，按优先级和故事点容量筛选。
  - 日报正文为摘要编号格式，草稿独立持久化，支持 Ctrl+D 一键格式化与 Delete 清行。

- Ctrl+Space 统一命令面板（受限功能，暂不开放）：
  - 新增全局命令面板，聚合命令、页面导航、产品包与文件搜索，支持模糊匹配和拼音首拼。
  - 产品包支持二级操作菜单，覆盖更新、解锁、打开 Revit/目录、调试、校验、定版。
  - 文件搜索复用 MFT 共享索引实时返回结果，并可桥接到 Ctrl+E 完整界面。
  - 命令面板启动后后台预热，消除首次唤起的加载黑屏。
  - 支持操作与包名组合命令，按可见产品生成组合项，回车用最新版执行。
  - 组合命令支持启动指定 Revit 版本，并修复版本号无法解析导致命令不启动的问题。
  
- 其他改进：
  - 工作日报与 Ctrl+Space 命令面板为受限功能：入口照常注册，暂不开放，仪表盘卡片置灰禁用，热键触发不响应。
  - 代码工作区整体对外可用，仅工作项详情中的 AI 实现/AI 修复与 AI 拆解按钮按特定账户隐藏。
  - 升级 Revit 激活工具到 1.3.4.4，资源释放改为版本感知，修复升级后仍运行旧版的问题。
  - 收紧 git-svn 提交日志 skill 的评分与条目长度规则，Base64 计算禁止创建临时文件。
  - 清理无用的快照文件。

- 提交参考：
- 7de54748b8e50a71909ea29a26d6b18f91bf12cc — fix(PackageManager): 子工作项按编号尾号重排对齐网页端顺序，同步 MftScanner 工具产物
- d2be74da4a6b42ba7e87acd0d284fccd096f3ef8 — feat(SubmitDefect): 示意图改走 PingCode 登录后的 atlas 永久链路，附件扩展到任意文件
- 30830ada0fdcdac25ff2ebf3f05add9fd7db6606 — feat(SubmitDefect): 提交工作项页支持视频附件并改为双列布局
- 26c9446f574cdaea396e6ea49e32eadf2a56af7e — feat(PackageManager): 新增「提交工作项」页，粘贴群聊一键提 PingCode 工作项
- 84ccc10d18980652869add162c28cd8d6962edce — feat(PackageManager): MiMo 用量页改用独立登录窗口并修复会话相关缺陷
- 45d647dd0ddaf57e39682feff727bdc7075857da — feat(MftScanner.Core、MftScanner、PackageManager): 索引宿主死亡或假死时强杀重启自愈，新增宿主侧与 App 双看门狗
- dd91c782b8fb03ed0ae92b1ffd907844d1f2e5f0 — docs(git_svn_commitlog_generator): 明确 Base64 计算不得创建临时文件
- c9afe5a5c95c3341bdfb809d9c22e801fe5a3f42 — feat(PackageManager): MiMo用量页新增切换账号与用量明细
- 0bcbae3c8d779be2e22ef1c1096d4294cec81f53 — feat(PackageManager): 新增 MiMo 平台 Token 用量统计页面
- 51fe81cf20638b6def3b83e70981d2c1e6300fa4 — feat(PackageManager): 放宽候选过滤与改进记忆文件管理
- d60e256bc0eef4661e7342e17d5c592007b3ad48 — chore(PackageManager): 将全局 AI 行为规则同步改为项目级记忆
- 96d96b3ec6c658378b9b42832b28e339572da1b9 — fix(PackageManager): 升级 Revit 激活工具到 1.3.4.4 并修复升级不生效
- 5f7dedfe5649eaa21c673606dd8e6e54a099af48 — chore(PackageManager): 收紧提交日志长度规则并刷新 MftScanner 产物
- 80588e15a949626d03c700d18ff6eea635f5ca28 — feat(PackageManager): AI 全局指令新增图片理解兜底规则并修正同步跳过
- 06f549d16593931cc61ecc62306da8788ebb24cb — fix(PackageManager): 修复命令面板打开 Revit 版本无法启动的问题
- 3877b4171f346a606783c79354db87a3ab384e27 — feat(PackageManager): 命令面板组合命令支持启动指定 Revit 版本
- ef4c3b30bee8cce4560a21c0537464e7a1a36518 — feat(PackageManager): 命令面板支持操作与包名组合命令
- d4f1acace093549eaf4d2e4dcb0dabf97e5b0a9d — docs(git_svn_commitlog_generator): 收紧条目长度评分规则
- 2337f810be50aaead2b3a53d8a8e85b40c947c28 — feat(PackageManager): 命令面板增加包操作命令与冷启动预热
- d2d1168e8498fb812ddd424c06c49171aeedee3d — refactor(PackageManager): 全局行为规则改写到 ~/.claude/CLAUDE.md 以真正生效
- 9a02f7ce1a8038b4a727021d50f46613b0972bb1 — feat(PackageManager): 新增 Ctrl+Space 统一命令面板，聚合搜索与包级操作
- 537126d49174aae3a32871be5e25b5695da9df9b — docs(SKILL.md、MftScanner.Core.dll、MftScanner.exe): 更新内嵌提交日志规则与构建产物
- 804a3db6b9edef3f5df77919b11180f2cbd8c882 — feat(PackageManager): 同步 AI 全局行为规则与记忆初始化
- 7ebdf24b9a0840b140e06db29f166eff27c0e02d — 删除无用的快照文件
- 07c769860bd8d4d5b76631e74b9ad335695dc438 — feat(PackageManager): 新增 AiMemoryService 并重构代码拉取为步骤模型
- b7572831984cdd78e14d55193de1a8899aece8f1 — feat(DailyLog): 明日计划按故事点容量筛选，日报编辑器支持 Delete 清行
- aa22d2a75dc37b220eeef1534543088576fcf37b — feat(DailyLog): 完善工作日报采集与格式化
- 8a7c60585cb4ec1bc153f7bf7458fd63cf2a6971 — feat(PackageManager): 完善工作日报采集与草稿恢复
- 11367a48da13ac3ee2888d79e5b30ad1e2ab0195 — fix(PingCode): 复制 Prompt 也走完整资源准备链路
- 560aaa8104e4190bea57e57344d7aa57e83e2bd6 — feat(PackageManager): 新增工作日报生成入口
- 4c1a180bd5782b715c1acfbf7f2eebf509c66509 — fix(PackageManager): 修复 MftScanner overlay 快照基准失配后的重试恢复
- d305f34926ca4324d008a5808d28031be088aaf1 — chore(PackageManager): 更新 MftScanner.Native.dll 并新增索引竞态查找测试
- 2dde321725aef29292d1f73cba6576b2e4e80b53 — fix(PackageManager-MftScanner): 修复 native 修复窗口内删除变更被旧快照恢复的问题
- 65181c7d9f438848830ed32f73807fb538667268 — feat(PackageManager): 预热 AI 提交环境并串行仓库操作
- d21c841486f5359d90176a3b0b6cae034a7bdad0 — feat(PackageManager): 自动同步 CodeGraph MCP 配置并更新 MftScanner 工具
- c06111f270b4b990790e54b61b2ee401bd8c7f00 — feat(PackageManager): 细化 CodeGraph 探索协议并更新 MftScanner 工具产物
- 4893f1316476b230a7c3ac85b2a0f430777101f2 — fix(PackageManager): 稳定提交流程的路径选择
- 94a989117f65c6fba2a12c142535a01fbb140fbc — feat(PackageManager): 同步 AI 全局 CodeGraph 规则并更新启动流程
- 052b6096d89d5d54a2efc6ffcae526240976f7f1 — feat(PackageManager): 预下载 PingCode 内网原型资源并写入 AI Prompt
- 9bfcb4a2989f08d538d713183342e2c41a8650ad — feat(PackageManager): 增加 MFT 单卷 USN 自动修复并隔离工作项图片
- 9db8b5b947438f44c2e16a1cfbc33f2240aec003 — feat(PackageManager): 增加 AI CLI 权限模式配置并同步 MftScanner 工具
- c3079ce59a6a72036aef26378db9558f30e479d9 — fix(PackageManager): 隔离 AI 提交流程状态目录，避免并发提交互相覆盖
- 1a879d2fd7b1853ad85f77af4da5c2c8d149f236 — fix(PackageManager): 调整 AI CLI 默认权限，避免跳过审批
- 26fa06761ed6a6f96e57487f75a785eb5d1a88cd — feat(PackageManager): 注入 CodeGraph prompt 协议
- b1514fcf76a2e999ad48f086aff9aa81fc91e14b — perf(MftScanner.Native、PackageManager): 重构 overlay 压缩决策，支持快照锁外构建与强制重试
- e247da2a6a192bfb1c51faf93e461742844e23db — feat(PackageManager): AI prompt 文件化管理与 PingCode 拆解去重增强
- bbb799ef330324306e832251cd078d127f2c145d — Mft性能优化临时提交
- 7cc2fd336235df96bef5bdbd74d0ce53a8cf7e16 — perf(PackageManager-MftScanner): 增加 prefix4 前缀索引，加速 native v2 前缀查询
- f7c70228b51d5ff19c6ef2fbd2c716a2944cb419 — feat(PackageManager): 增加 PingCode AI 拆解和终端启动模式配置
- 3346d691d25301ff3559513aeaf2e420806ebb67 — feat(PackageManager): 增加 CodeWorkspace MSBuild 构建入口
- aad3c420ac2664ee1615322065090c15bc8326f7 — fix(PackageManager): 隐藏受限用户的 PingCode AI 操作入口并同步 MftScanner 工具产物

## 4.2.0.0 — 2026-06-02

- 代码工作区 VCS 状态与差异查看：
  - Git/SVN 状态展示增加仓库类型、分支版本、变更数量和 SVN 子仓库状态面板。
  - VCS 状态支持 Git 子仓库检测与展示，根仓库和子仓库分别维护前后追踪状态与变更计数。
  - 工作区本地状态与远端更新刷新拆分，先显示本地状态，再后台检查远端统计，减少常规等待。
  - 差异查看从 DiffPlex.Wpf 改为自研 ListView 控件，支持行号、变更标记、字符级高亮、水平滚动和空白变更检测。
  - 差异文件大小上限从 128KB 提升到 1MB，外部差异工具支持自动创建空基准并增强路径校验。

- 产品包关联与项目文件选择：
  - 代码工作区支持为源码仓库绑定、打开和解除产品包关联，并在仓库详情中显示关联状态。
  - 产品包首页新增源码仓库入口，可按当前产品包绑定仓库或跳转到已关联仓库。
  - 关联选择窗口同时支持产品包和源码仓库选择，并按名称与路径提供自动建议。
  - 项目文件扫描同时采集 `.sln` 和 `.csproj`，选择对话框支持按类型过滤、状态栏提示和确认按钮状态绑定。
  - 仓库管理和工作区页面的项目文件扫描改为异步执行，失败时给出明确提示。

- 合并回主干与 AI 提交环境：
  - CodeWorkspace 新增合并回主干入口，支持根仓库与 Git 子仓库选择。
  - 合并流程增加预检、fetch/pull、merge、冲突继续和终止操作。
  - 冲突文件列表支持生成 AI 分析提示，便于处理合并冲突。
  - AI 提交入口改为后台检查命令与同步 skill，避免界面被环境准备阻塞。
  - AI 提交入口为每次运行生成专属状态目录，并要求 Step 3 显式读取本次 `last_changes.json`，避免两个模型并发提交时共享 `.state` 互相覆盖。
  - 用户级/手动执行 git-svn 提交 skill 时，Step 1 默认改为在当前仓库 `.pm-ai/commit-state` 下创建唯一状态目录，并通过 JSON 输出供 Step 3 显式复用。
  - 已记录一次并发状态覆盖异常：首次 Step 3 误读被 `E:\PackageManager` 覆盖的共享 `.state/last_changes.json`，导致 PackageManager Git commit `ca6a601` 已提交并推送到 `master`；未手动回滚，后续改用独立 `StateDir` 与显式 `-ChangesJsonFile` 完成 MaxiBIMSH 的正确 SVN 提交。
  - 打开 IDE/Visual Studio 前异步选择并扫描项目文件，失败时保留明确状态提示。

- PingCode AI 执行入口与内网资料访问：

  - 工作项详情页新增 AI 实现/AI 修复入口，按类型切换文案，并补拉详情后打开执行窗口。
  - 新增 Prompt 提炼和图片下载，汇总工作项描述、评论、扩展字段与参考链接。
  - 执行窗口支持选仓库、编辑与复制 Prompt，并启动 Claude 或 Codex。
  - 图片随执行写入仓库 `.pm-ai/images`，CLI 启动服务支持超长 Prompt 落盘。
  - 支持识别内网链接、下载 Axure 子页面并内联图片资源。

- 通知、传输与工具产物：
  - 通知服务支持保存历史，保留最多 200 条有效通知，重启后延续未读数量与列表。
  - 通知弹窗支持点击外部关闭，并保持铃铛点击与未读角标同步。
  - 局域网文件传输新增自动接收、传输完成确认、跨卷落盘回退和占用文件重试处理。
  - 提交日志生成规则继续收紧，支持轻量状态文件、反馈重生成和按真实 Git/SVN 提交组聚合。

- 提交参考：
- 92b6904e5dc0506626aed893a8c7e904a5a55021 — feat(PackageManager): 完善 PingCode 内网资料访问与 AI 启动准备
- 23dd9e2740b724251010b9a1711b6186db7a09ca — feat(PackageManager): 在工作项详情页接入 PingCode AI 执行入口
- 0fe3d4047dda8f94d44f1abfc6979f2564a1e294 — feat(PackageManager): 增加代码工作区合并回主干流程
- 8dcc65d9ff654fbfb3d0739c332c9c4f999b5e19 — Merge branch 'refactor/architecture-v2'
- 08185c443d3aee05d3b30c95867f3dfe006249e2 — fix(PackageManager): 异步准备代码工作区外部工具与 AI 提交环境
- 7abe8817a98f54bcb897a0c2ddd263c17844babe — feat(PackageManager): 持久化通知中心历史并同步工具产物
- abff84ea87a627a2dbcb911ae147f9a868393a64 — feat(PackageManager): 项目文件选择支持 sln/csproj 切换，扫描改为异步
- 12267e1a0c59e2dbb41fed0af10a02dde9617413 — feat(PackageManager): 增加产品包与源码仓库双向关联
- 8c2e0ce8a380d0ba3476c6f113727fcbd233bc8b — chore(PackageManager): 收紧提交日志规则并同步 MftScanner 工具产物
- d151304c103e6fd8f82aa7d0fbe8dab1499e9ab7 — feat(PackageManager): 拆分工作区本地与远端状态刷新
- ab2c9f25dfcaf6c102951502a692df6022c7e1a7 — feat(PackageManager): 增强工作区版本状态展示
- 3808842c7fe68214569f41ed5ad81e725aaf5661 — feat(PackageManager): 支持按真实提交组生成日志
- a3ab28662e52966822575b5e0a22ae37c4937972 — 临时提交
- 2523f9508e58b25b09c5bf19fb17d5a5591133c6 — feat(CodeWorkspace): VCS status 支持 Git 子仓库检测与展示
- 8a15ebce2d32e6d259a9cd2603b0b593e5474aad — feat(PackageManager): 代码工作区差异查看改用自研控件，支持字符级高亮与水平滚动
- 1141857449e890843bbf29a25970440f66fe6f4f — feat(PackageManager): 代码工作区新增 VCS 变更差异查看功能
- 4fe5b6968a06e85bee416a0a982cd515a223f2ea — feat(PackageManager): 增加代码工作区 Git/SVN 状态展示
- 6a449b78e096139ba2e70e2f59d0ea5d73d3c62d — build(PackageManager): 更新 MftScanner 工具二进制并收紧提交日志生成规则
- 105ca9a540f3c402d8269efd0dceda7af17d788d — refactor(git_svn_commitlog_generator、MftScanner): 简化输入处理并细化日志生成规则
- ae2605d6e1e3d93131519f6c420881ebb7b44496 — feat(PackageManager): git-svn-commitlog-generator 新增轻量状态文件与反馈重生成流程
- bf0a0058998bdad1974ed16ddfbfa85029977087 — feat(PackageManager): 新增自动接收与传输完成确认，增强接收落盘健壮性

## 4.1.0.0 — 2026-05-27

- 通知中心与仪表盘概览：
  - 新增通知服务、通知模型和通知面板，支持未读计数、全部已读、清空与点击导航。
  - 主窗口顶部接入通知铃铛和弹出面板，未读数变化时同步更新徽章。
  - ToastService 将现有 Toast 同步写入通知中心，应用启动时注册 NotificationService。
  - 仪表盘新增包状态、今日通知、LAN 节点概览，并支持刷新和跳转。

- 包版本监测与更新控制：
  - 新增定时轮询 FTP 最新版本，记录新版本数量并推送通知。
  - 包信息结合服务器版本展示可升级状态与上传时间。
  - 应用启动注册并启动版本监测，进入产品分类页时刷新 FTP 可见版本。
  - 可见包版本默认保留当前选择，刷新按钮改为 forceLatest 模式，手动刷新时切换到最新版本。
  - 新包检测支持同版本按时间提示新包，并保留上传包选择。

- 代码工作区与 AI 提交入口：
  - 新增代码工作区与功能模块，包含 工作区和管理仓库 及三个视图页面。
  - 仪表盘增加代码工作区卡片入口，注册代码工作区导航页。
  - 仓库操作列由单一提交按钮拆分为 Claude提交 和 Codex提交，并扩大操作列宽。
  - 提交流程按引擎校验命令并复用内嵌 skill 同步与提示构建。

- 工具二进制与启动稳定性：
  - 修复启动台首次日志写入失败，LoggingService 写入入口先初始化日志目录。
  - 修复常用启动项 Enter 启动前 LogInfo 抛出 path1 为空的问题。
  - 多次同步更新 MftScanner.Core.dll 与 MftScanner.exe 二进制资产。

- 提交参考：
- 10d61bcfeedd5370d176a1eebef6bc913f8847dc — feat(PackageManager): AiCommitSkillInfo 新增 LastChangesJsonPath 属性，SKILL.md 改用 内嵌绝对路径
- 5116222f3b8e867defd587dac884a586a1497a39 — feat(PackageManager): 拆分代码工作区 Claude/Codex 提交入口
- 19365ef2379477cfea8aea8505bd721a5085dc21 — refactor(PackageManager): 重构 AI 提交 skill 为内嵌直接执行模式，新增快捷方式解析
- 97a7e59cee412cb26e02dbf3ae35c5b421f73e18 — feat(PackageManager): 新增代码工作区模块，集成仓库管理与 AI 提交技能
- 91bac969d5f64637c6c763493d3f480739cfbb9f — docs(git_svn_commitlog_generator): 增加输出前自检与 Python chr() Base64 兜底
- 9d0b7325735a9b0ccdbfa64a184cbec2a1d20fd7 — docs(git_svn_commitlog_generator): 提交日志默认紧凑输出，收紧换行触发条件
- 13455ed377e2c7ede7084f9833b1d7c3abb49852 — docs(PackageManager): 补充 Base64 失败重试规则
- 94af9338b98b43140285c3fde7767f2c762ba4a6 — docs(PackageManager): 补充提交日志的范围收敛与分组约束
- ed433eccf771e095187208891b0f2d065a162f2d — fix(PackageManager): 修复新包检测
- cd5f9df5bccda7deb3b34d43e39bd69a02cdff2f — feat(PackageManager): 增加服务器最新时间显示与版本刷新控制
- 6ba4e75ac0601be46107bf9e6c0b53792acf7cd5 — feat(PackageManager): 增加包版本监测与更新概览
- c52efd60716de6bf5bdf03760dafae7d71533055 — feat(PackageManager): 增加通知中心与仪表盘概览
- b14eb5f4469993b9231ec7546ecfca47b01ac927 — fix(PackageManager): 修复启动台首次日志写入失败

## 4.0.0.0 — 2026-05-25

- DevKit 仪表盘与导航体验：
  - 新增 DashboardPage 仪表盘首页，启动后默认进入 DevKit 工作台视图。
  - 仪表盘按包管理、项目协作、开发工具、日志和设置组织入口，支持通过卡片直达各页面或弹窗工具。
  - 主窗口移除左侧导航栏，中央内容区改为全宽显示，并在非首页显示返回仪表盘入口。
  - 返回仪表盘入口改为图标化按钮，包管理工具栏仅在产品分类页显示，减少非包管理页面干扰。
  - 更新日志调整到日志分组，插件管理归入包管理区域，Revit 破解入口提升到仪表盘。

- 包管理与开发工具入口：
  - 产品分类从启动首页拆分为独立包管理页面。
  - 主窗口可按当前页面切换工具栏可见性。
  - 统一承载 CSV 加解密、清理 RVT、解除占用、编译顺序、Git 代理、VCS 映射和 Revit 激活工具启动逻辑。
  - 包管理页移除通用工具按钮，保留产品相关操作，避免工具入口重复。
  - Git 代理卡片在没有状态回调时弹出结果提示，避免点击后无反馈。
  - 修复包列表为空时打开页面抛出异常的问题。

- 工程结构与领域重构：
  - 将功能代码迁移到 Features 分层目录，按包管理、日志、PingCode、设置、局域网传输和 Revit 工具等业务域归集。
  - 统一功能注册、导航和服务实例管理。
  - 更新工程引用，适配新的 Features、Infrastructure、Shell 路径。
  - 保留主页、设置、日志、产品管理、看板和文件传输等页面入口的导航同步与回退逻辑。

- 提交日志工具增强：
  - Git/SVN 提交日志工具支持嵌套 Git 仓库、多提交组日志和按组提交/推送结果回传。
  - 提交推送脚本支持无 upstream 时自动执行 `git push --set-upstream origin <branch>`。
  - 文件选择对话框支持 Shift+Click 范围勾选，交互操作会重置倒计时；控制台回退输入支持范围语法。
  - 提交推送时可安全识别并恢复陈旧 `index.lock`，失败时保留锁路径、锁龄和相关进程摘要。
  - 多提交组提交前校验每组专属日志，避免把默认日志误用于其他仓库或工作副本。
  - 调整提交日志紧凑写法规则，短条目保持单行，长技术项按阅读负担换行。

- 索引宿主兼容诊断：
  - 索引宿主计划任务注册改用 System32 下的 `schtasks.exe` 并固定工作目录，降低 Win10 受限启动目录导致的拒绝访问风险。
  - 捕获并记录 schtasks 和管理员提升流程的 Win32Exception、NativeErrorCode 与退出码。
  - 启动失败提示补充系统策略或安全软件阻止创建计划任务的可能原因，便于定位环境问题。

- 提交参考：
- 4c1b6483ff6fa550da43639ef478fc8c94814561 — feat(PackageManager): 完善仪表盘工具入口与页面工具栏控制
- 4838a20cf9e041911c973b45a59b5ad34ba0a547 — fix(PackageManager): 调整仪表盘导航布局与工具反馈
- 36031c5598da79a00e05f9218d76b69d99e53457 — feat(PackageManager): 增加 DevKit 仪表盘首页与分组导航
- 34b5f49c5ae6c07b14f2717fe19cec4462d9217b — refactor：删掉原先的文件，按领域划分功能重构已应用
- c2a390b676be90617fa8bf2ee99cd43807d7688e — refactor(PackageManager): 重组功能目录与导航服务
- be62bcb9b6f22522ceebab637357c68c7f5257a8 — feat(git_svn_commitlog_generator): 支持自动设置 Git push upstream
- 8da0ea9aee5c1200e7075b3e7648f91e0be35558 — feat(git_svn_commitlog_generator): 文件选择对话框支持 Shift+Click 范围勾选与交互重置倒计时
- 4cc87ec46c3cf0fd128791606635df562c7b8db4 — docs(git_svn_commitlog_generator): 调整提交日志紧凑写法规则
- 4dba43787cd0956a618f5cec16f06320475a2acc — fix(git_svn_commitlog_generator): 提交推送时安全恢复 Git index.lock
- 5d3efc831999a4d36d5789e209bf60e2abe2286b — fix(git_svn_commitlog_generator): 校验多提交组专属日志并更新说明
- 1c469459f9b9ccecde410b2918872b8fbeb88311 — fix(PackageManager): 增强索引宿主计划任务注册的 Win10 兼容诊断
- d8e7a028437b4229273ecc31763a89be1e042212 — feat(git_svn_commitlog_generator): 支持嵌套 Git 仓库与提交组日志

## 3.3.0.0 — 2026-05-20

- MFT Native v2 索引与查询性能：
  - 新增 Native v2 MFT 加速索引，围绕短查询、路径范围查询和快照恢复重构核心搜索链路。
  - 为单字符、双字符、短 contains 查询增加专用桶、位图、计数表和盘符统计，降低短词查询全量扫描成本。
  - 路径范围查询改为目录 recordId 定位、盘符过滤、父目录 postings、目录子树区间和 recordId 顺序表组合过滤。
  - 大路径范围短查询支持在子树区间内分页计数，覆盖 `C:\Windows`、`C:\Users`、仓库目录等场景。
  - 根路径和盘符限定查询补充正确性校验，修复限定 C 盘时无法搜到 C 盘内文件的问题。

- 快照恢复与 SLA 压测：
  - Runtime 快照升级到 v6，持久化子树 enter/exit 区间与 recordId 顺序表，旧快照自动重建兼容。
  - 快照保存改为后台维护并降低独占锁干扰，保存阶段使用堆上副本承载大数组，减少栈和拷贝压力。
  - 全量构建完成后先发布可搜索状态，records remap、快照保存等后续阶段异步执行，降低 ready 阻塞。
  - 压测脚本补齐冷构建、恢复 ready、端到端查询、宿主耗时、锁等待、路径越界和返回错配统计。
  - Release 压测覆盖冷构建、快照恢复、全盘查询、路径限定查询、根路径查询和额外大路径抽样。

- USN 增量与 overlay 稳定性：
  - 增加 USN 批量 live-delta 增量更新，统一应用创建、删除、重命名和恢复记录，降低频繁重建成本。
  - USN Journal 过期时支持单卷 MFT 重新枚举并合并替换对应盘符记录，尽量保留现有索引继续服务。
  - Overlay 记录覆盖稳定索引时按 key 抑制旧记录，查询结果按路径去重并修正物理/唯一命中统计。
  - 后台追平区分成功、过期、取消、拒绝访问和读写失败，快照保存与 contains postings 恢复均校验内容指纹。

- 索引宿主、IPC 与启动项体验：
  - 后台索引宿主改用 MMF IPC，并增加工具包指纹校验、宿主同步和启动失败提示。
  - 搜索请求改为异步调度并支持过期响应丢弃，避免 Ctrl+Q 搜索和索引状态变更触发刷新风暴。
  - 搜索结果支持加入启动项分组，右键菜单和工具栏可直接写入 common startup 配置。
  - 脚本启动项支持 PowerShell 7 终端运行，Ctrl+Q 工作台增加 Ctrl+T 终端入口。

- 局域网文件传输：
  - 接收文件直接提交到收件箱根目录，保留发送端顶层文件和文件夹结构。
  - 新增同名时静默覆盖设置，支持同名文件覆盖和同名目录替换。
  - 接收确认窗口显示真实收件箱目录，不再展示时间戳子目录。

- 提交日志与工程工具：
  - 新增 git_svn_commitlog_generator 技能，支持 Git/SVN 待提交改动采集、交互排除/加入和提交日志生成。
  - 提交推送脚本支持限时交互、Git 自动 commit/push、SVN 按仓库合并提交和 PowerShell/Windows Terminal 启动。
  - 优化大工作区状态采集、未跟踪项过滤、SVN XML 状态读取、CRLF 警告处理和路径规范化。

- 提交参考：
- 69b23039a187dba03c0566799cbd3b4c4ad05a3a — 性能(PackageManager): 用子树区间优化 MFT 大路径范围短查询
- e0cba0d3e80e79838d4ef9ea488c9709d6e30357 — 修复(PackageManager): 优化 MFT 短查询路径范围过滤并补充根路径校验
- e0f452561fb94a7dc351aae638ebeb4ebb7213b9 — feat(PackageManager): 优化文件传输收件箱落盘与同名覆盖策略
- 545a7d48eecb97f94c9b8da4d4109d08b439cab2 — perf(MftScanner、MftScanner.Core、PackageManager): 修复共享索引重建 SLA 与进度反馈
- 4a30c12d11f7abadaf244cdc06fc6c6699c6d45a — fix(PackageManager): 修复索引宿主启动失败和 CtrlQ 搜索刷新风暴
- 6b33ade4ce3a5590108d3c220cba1e16621f2933 — fix(PackageManager): 修正 Native overlay 追平与压测统计口径
- f6e317d83a919e2c8011f513f3120e30bb897dd7 — perf(MftScanner.Core、PackageManager): 优化 overlay 路径范围查询并补齐冷构建压测指标
- 3f26c9135be13ed22fca2f6ad13a60b68083bd7b — feat: Native 枚举区分 FSCTL_ENUM_USN_DATA 正常 EOF 和异常退出
- 0d7c251cbd15b3f540d4ec1aa3157bf6447e0361 — feat: optimize native v2 index build and search
- 076a2127cf9cb4a31f4b0b97754b00912098506c — feat: add native v2 mft index acceleration
- 293fe015bc34516f3ab7a2465b6012ae931c4094 — perf(MftScanner.Core、PackageManager): 优化短查询索引快照与路径前缀搜索
- d80a40f89da766dcdd469bd24742a165983b87c2 — feat(MftScanner、PackageManager): 支持搜索结果加入启动项分组
- 026ef8231897478d41236d358055f1627bce4394 — fix(MftScanner.Core、MftScanner、PackageManager): 增强 USN 过期恢复与搜索热结构预热
- 19c69fad7ced4de83a0be1cf3eaca7c8631e37f1 — fix(MftScanner.Core、PackageManager): 稳定快照恢复与 USN 追平流程
- a5753ceaf59431e83fe2dcb4d0f76ba5aead534a — perf(MftScanner.Core、PackageManager): 优化 Contains 短查询加速与验证覆盖
- 807554c4f9d70b8aa8be9efd786a54f87705a751 — feat(MftScanner.Core、PackageManager): 增加 USN 批量 live-delta 增量更新
- 23e86554db18b5700d35212e5c789f3bb7a6f1f0 — feat(PackageManager): 脚本启动项支持终端运行，命令改用 EncodedCommand 传递
- 89c0b91fbb8b5921bfadba192c419fb06e999a6e — feat(PackageManager、MftScanner): 终端改用 PowerShell 7，启用时清理禁用残留
- 7064836b3fb3ec891432ebcfebc0d97cf499612e — refactor(PackageManager): 插件启用/禁用从重命名策略改为独立目录隔离
- a7fd26ba730d187095c063513652b5db833f66c6 — perf(MftScanner.Core): 双字符 ASCII 计数索引与增量更新，ApplyBatch 读写锁分离
- 92c7c0ae5c6cdfd693f85e7a9b478e6f0ef91c0d — perf(MftScanner.Core): 单字符 ASCII 位图索引、驱动器+类型过滤搜索与路径前置策略扩展
- 8b72e209a484633843deaf96368e63059a14711a — feat(MftScanner.Core、MftScanner、PackageManager): 软删除覆盖层、搜索请求异步调度与路径前置过滤策略分级
- d480c8f66ee6a7633e48e8a12fd696e54b603803 — perf(MftScanner.Core、PackageManager): 快照 v7 列式分文件格式并行反序列化、Postings 恢复失败自动触发预热
- ec29d5bc52efa43c74ec74a49e67bd155da10269 — perf(MftScanner.Core、PackageManager): MFT 硬链接全保留、快照恢复延迟构建派生结构与独立 Postings 文件
- 4bc761618e539e5044c4f0a6f33c26b27c7e42b7 — feat(MftScanner.Core、MftScanner、PackageManager): 搜索结果按全路径去重、快照升级 Deflate 压缩与内容指纹、IPC 协议 v4
- e84050ce954a9deae5fffd0eddba7446e6866064 — perf(MftScanner.Core、MftScanner、PackageManager): 新增短查询热桶缓存、索引快照过期标记、搜索空闲等待与基准测试增强
- 4136ff3b65026bfd163b8c57389814f2bd52c97d — perf(MftScanner.Core、PackageManager): 移除 ExactHashMap 改用二分查找、ContainsAccelerator 增量压缩与目录专用 FRN 映射
- 7cb1c46a50ce9d5d0e9838ddd9dc5c6488d63b4e — perf(MftScanner.Core): 增量查询缓存、ContainsAccelerator 不可变重构与目录子树缓存优化
- 7cbe206320d24fd06b36d827bddbb4ea55db0914 — feat(MftScanner.Core): 路径前缀前置过滤替换后置过滤，优化 IPC 共享内存序列化
- f80d17b0c66af46d45f6e1434b95e3f631227496 — feat: IPC 通信改为 MMF，去除 NamedPipe 实现
- 822b1d29fa0faad2f0eb0d654cde86ca19c47980 — feat: 添加后台宿主索引服务的必要文件

## 3.2.0.0 — 2026-04-23

- 拼音搜索支持：
  - 添加拼音匹配功能，支持中文搜索（全拼和简拼）

- 搜索体验优化：
  - 优化搜索框文本变化处理，改为防抖模式，解决输入不跟手的问题
  - 改进搜索框键事件处理和方向键导航逻辑
  - 增加搜索结果导航快捷键支持并优化加载计数逻辑
  - 修复文件夹搜索同名文件夹被跳过的问题

- 键盘导航增强：
  - 添加路径限定的键盘导航支持并优化搜索框输入状态管理
  - 添加类型筛选键盘导航支持并优化类型筛选逻辑

- 局域网文件传输：
  - 添加局域网文件传输功能及设置选项
  - 重构局域网文件传输页面样式和布局
  - 添加局域网文件传输确认窗口及页面逻辑

- 启动项改进：
  - 添加启动项工作目录处理，脚本类型启动项从工作目录启动
  - 更新设置保存逻辑并分离通用启动项设置

- 窗口与热键：
  - 提升 Ctrl+Q 和 Ctrl+E 前台激活成功率并新增调试日志分类
  - 添加索引服务性能分析日志功能

- 提交参考：
- 9bef710edeb197b89c18e1fbb0d6478883db7721 — feat: 添加拼音匹配功能以支持中文搜索,支持全拼和简拼
- 9ed10937a4937915ee4cf7d794ef908044de422e — refactor: 优化搜索框文本变化处理逻辑，改为和文件搜索一致的防抖
- 61031953f13fee590548e16da4fe9c5b5fba606b — feat: 增加搜索结果导航快捷键支持并优化加载计数逻辑
- 2fdc8a3b13755ed3f2429770c29e817ca35e7f92 — fix: 修复文件夹搜索同名文件夹被跳过的问题
- 57ad5593fc271bd91f9f486c56041eec8d00255a — feat: 添加索引服务性能分析日志功能并支持嵌入式工具侧载
- 1b100b0905b0de779af9e1b4e52c1420f326c92f — feat: 添加局域网文件传输功能及设置选项
- 20aa086f3196212f8d601929abb5cacd4f9c93da — feat: 添加局域网文件传输确认窗口及页面逻辑
- edf4130c1f722b1c8d2ee82e322f1b5e7db56b61 — feat: 提升CtrlQ和CtrlE前台激活成功率并新增调试日志分类
- 74235c5b2e9b4f5e5ff70b6a399d6c510254e4a7 — feat: 添加启动项工作目录处理,脚本类型启动项从工作目录启动
  
## 3.1.0.0 — 2026-04-21

- MFT 文件搜索功能集成：
  - 新增 MFT 扫描机制，支持首次全量 MFT 枚举和后续 USN 增量更新
  - 添加无头 CLI 模式，通过内存映射文件（MMF）与主进程通信
  - 实现磁盘缓存支持和强制重扫功能
  - 支持路径范围限定查询和通配符模式匹配
  - 搜索结果过多时支持分页加载
  - 文件检索支持文件夹
  - 优先保留长文件名并处理循环引用
  - 集成 Costura.Fody 嵌入依赖项

- 常用启动项升级为工作台：
  - 新增工作台 UI 界面，替代原常用启动项窗口
  - 支持分组管理启动项，编辑启动项后切换分组不跳转到该分组
  - 添加搜索功能，搜索时自动切换分组逻辑
  - 搜索前状态快照，临时搜索结束后恢复之前的分组和滚动位置
  - 支持方向键导航和启动项选择，提升操作体验
  - 添加启动项持久化加载和保存逻辑，增强数据备份和恢复功能
  - 修改候选项列表样式，优化滚动交互
  - 修复工作台区域列宽无限延伸的问题
  - 添加工作台滚动优化和刷新逻辑改进

- 全局热键与窗口管理：
  - 注册系统级热键，避免快捷键在其他前台程序内无法唤起
  - 添加常用启动项窗口管理和全局热键服务（Ctrl+Q）
  - 支持窗口关闭隐藏和单实例管理
  - 添加周期性快照保存和会话管理

- 索引服务优化：
  - 重构 MftScanner 索引服务，优化索引构建速度
  - 添加快照+USN 追平方式启动，提升索引恢复速度
  - 实现持久化快照和批量更新 USN
  - 修复 UsnWatcher 只监听一个卷导致某些文件事件没捕获到的问题
  - 优化 USN Rename 事件响应，避免丢事件
  - 在索引服务层加入扩展名预筛选提升文件搜索和启动项过滤性能
  - 后台 USN 追平和搜索结果处理优化
  - 先过滤启动项再筛选，避免排在后面的真实结果没显示在前 500 条
  - MftScanner 结果列表卡顿问题解决

- 文件搜索升级改造：
  - 文件搜索 MftScanner 升级改造和 UI 交互设计
  - MftScanner 交互优化
  - 移除文件搜索功能的用户限制并更新相关提示信息
  - 修复先按桌面搜索完切换可执行再切换全部类型时无法搜索到内容的问题

- Revit 功能扩展：
  - 对打开 Revit 功能进行扩展，已存在进程时提供多个选项

- 其他改进：
  - 使用 FolderPickerService 替换 FolderBrowserDialog
  - 将 MftScanner.exe 重构为 dll 形式
  - 调整按钮样式和布局

- 提交参考：
- b4eaf57c4a12cca28dfb38b1d3040224f82356d2 — feat(MFT)：增加MFT扫描机制
- 7f4460a306018d93d43202ae88e904ec9d581c56 — feat(MftScanner): 添加无头 CLI 模式和 MFT 扫描工具集成
- da7739d571fb15dbf4dca60b5bdb7430d0c42d91 — feat(MftScanner): 添加文件搜索功能和自测模式
- 899a4231535d7accf5cb2b2d18a76e49bf83d6ed — feat(StartupTool): 添加常用启动项窗口
- 271ffe7cc982e0591acd6311448e2088ee9c00e7 — feat：重构MftScanner的索引服务
- e81aa37556bec6a95aa125894b033e7b3ba543fa — feat:增加快照+USN追平方式启动，提升索引速度
- 47f57fec49b0710708e3e16f144a7394bd5b05e1 — fix：修复UsnWatcher只监听一个卷导致某些文件事件没捕获到造成不能实时更新搜索结果的问题
- 7f1cd75b5ebfd800a87b18add07ac1410e032767 — feat:将常用启动项升级为工作台
- d45ad3bbad655c8f654d473aa22854c46bad94de — feat: 添加常用启动项窗口管理和全局热键服务
- f8644a937daca67aa46be8d48c5e07ab9f183321 — feat: 添加常用启动项窗口关闭处理和单实例管理
- 113ea56fb43e6b13a1af72b9e24e202bfbb2cdd2 — feat: 添加周期性快照保存和常用启动项会话管理
- ca456fbd57f3aeade157557d905f3109be7ab353 — feat:对打开Revit功能进行扩展，已存在进程的情况提供多个选项
- b44dec1c42ad9f9a3e5731f8ba8fd04d824bff38 — feat：注册系统级热键，避免快捷键在其他前台程序内无法唤起
- e0a633b6e596586a3f1dce65a6959d0906dad4fd — feat: 添加工作台滚动优化和刷新逻辑改进

## 3.0.0.5 — 2026-04-08

- Revit 文件清理增强：
  - 添加 Rvt 和 Rfa 文件快速扫描和删除
  - 实现 Everything 索引提供程序用于快速文件搜索
  - 开发本地索引提供程序支持实时文件监控
  - 创建统一的文件查询模型和进度跟踪机制
  - 添加索引存储管理支持持久化和原子写入
  - 实现文件路径规范化和扩展名验证功能
  - 集成文件系统监控器支持增量索引更新
  - 优化 UI 显示索引状态和进度信息
  - 支持重建索引功能
  - 实现文件变更实时同步到索引

- VCS目录映射：
  - 支持输入根目录更新和补充目录的版本管理

- 提交参考：
- b4b6bd304ab5c5934f34a80af09c0f683e6f4ffd — feat(PackagesHomePage): 支持调整vcs目录映射 
- 28057fd2e2d74085df3d8f4f5c5f644ed1020801 — feat(revit-cleanup): 添加文件查询索引功能
- 432f5d8de18ad429af76ae4c186b10f4a943ff17 — feat(revit): 添加Everything索引和本地索引功能
- 64b1faeb0273a22d2dcf0f86632cb35b13a04f96 — feat(services): 增加Rvt和Rfa文件快速扫描和删除

## 3.0.0.4 — 2026-04-07

- 包选择逻辑优化：
  - 在应用程序初始化和产品过滤后确保最新活动包被选中
  - 修复多产品下载时抢选中项的问题

- 插件配置下发时机调整：
  - 调整下发 addin 的时机，改为包下载解压完成后执行

- Git 代理管理功能：
  - 添加 Git 代理切换功能支持
  - 添加 Git 代理地址配置项
  - 实现 Git 全局代理的启用、禁用和状态查询

- 开发工具增强：
  - 添加 PowerShell 脚本用于获取最近提交的 C# 文件
  - 支持 Git 和 SVN 仓库的最近提交文件查询
  - 实现 C# 文件筛选逻辑，排除生成文件和设计文件
  - 支持按作者和提交日期过滤功能

- 代码质量改进：
  - 更新和完善代码注释

- 提交参考：
- e9c302b7c9284426233897efebc85af0567ee5bc — feat(main): 确保最新活动包被选中
- 52d994807e4c8a5534b04ca1b5db2dca5b59193a — refactor: 修复多产品下载时抢选中项的问题
- f75c89c00755693688f26fd15a6a3bf22c66588f — refactor(models): 调整下发addin的时机，改为包下载解压完成后
- 8f852b9e05e47daf17a103b46050be3ee15d3243 — feat(git): 添加 Git 代理切换功能
- 741714872360e2a89d01580904c24fe2c7df0a61 — feat(scripts): 添加 PowerShell 脚本用于获取最近提交的 C# 文件

## 3.0.0.3 — 2026-04-01

- 缺陷修复：
  - 修复并行下载取消失效的问题

- 代码质量优化：
  - 移除 重复的 Microsoft.Win32 引用
  - 改进异常处理并添加日志记录
  - 启用可空引用类型支持
  - 添加警告禁用标记以处理未使用的变量

- 提交参考：
- b3fe660e3626fb54c7732141f94cc70d217d0424 — fix(package): 修复并行下载取消失效的问题
- f96d1d434b5444c96d2c0cc1ebb99a1bb8aa86d1 — fix(core): 优化代码质量和错误处理

## 3.0.0.2 — 2026-03-16

- 下载性能优化：
  - 实现并行下载功能，支持大文件分片下载
  - 添加分片下载探测机制，自动检测服务器是否支持 HTTP Range 请求
  - 实现单线程和多线程下载两种模式，大文件自动切换到并行下载
  - 优化下载进度报告，显示百分比、速度、预计剩余时间等详细信息
  - 改进下载进度回调机制，提供更精确的进度跟踪
  - 添加下载速度计算和 ETA 预估功能

- FTP 服务增强：
  - 提取版本号语义排序功能
  - 实现按版本号语义排序名称的方法，支持任意位置的 vX.Y[.Z[.W]] 格式
  - 更新本地路径设置页面使用新的排序方法，修复本地路径设置页面版本顺序不正确的问题

- 提交参考：
- b790f199326e0b2d3aced2418605f6055db973ed — feat(PackageUpdateService): 实现并行下载功能并优化进度报告
- 31b149add5660a2386999463d97afe9b9b122698 — feat(FtpService): 提取版本号语义排序

## 3.0.0.1 — 2026-03-13

- Jenkins 编译功能：
  - 添加 Jenkins 构建进度报告功能
  - 添加 FromBuildResult 方法处理构建结果转换
  - 在 UI 界面中集成进度消息显示功能

- 提交参考：
- d0405fe7595ae1d4056032229b9f2e4dd073bb45 — feat(jenkins): 添加 Jenkins 构建进度报告功能

## 3.0.0.0 — 2026-03-12

- Jenkins 编译功能：
  - 添加 Jenkins 编译功能支持，实现 Jenkins 编译触发逻辑
  - 新增 CredentialProtectionService 用于保护凭证信息
  - 在设置页面添加 Jenkins 配置界面，包括地址、视图、用户名和密码字段
  - 实现密码加密存储和解密读取功能
  - 添加 Jenkins 编译结果状态显示和错误处理

- 界面布局重构：
  - 调整主窗口高度从 700 增加到 780 像素
  - 为进度条控件添加 IsVisible 属性并默认设置为 false
  - 将功能按钮重新组织为三个分组区域：主要操作、配置相关、工具与维护
  - 使用边框和标题对不同功能组进行视觉区分
  - 调整按钮尺寸和间距以优化界面布局

- 本地路径设置增强：
  - 在分组头部添加统一设置路径和清空本组按钮
  - 实现按产品名称统一批量设置本地路径功能
  - 添加分组路径状态摘要显示当前路径设置情况

- 缺陷修复：
  - 修复软件设置保存时会重置日志等级的问题

- 提交参考：
- bb5dee2ecdb0869535f3e8e284b83cdc03c00e79 — chore(version): 更新程序集版本号
- 2a67f7da1be95698581539510e357675f25e51b8 — feat(jenkins): 添加 Jenkins 编译功能支持
- fbd053e67b302e6434d2da46818ff44dab3f629b — refactor(ui): 重构包管理器主界面布局和数据模型
- bc3c59a3d5d751f54e5038495d04efe1fdd4fcad — feat(local-path-settings): 本地路径设置页面功能增强
- 03a5fe7274c2eebfa1bd7d9602afd2ccd6659863 — feat(SettingsPage): 修复软件设置保存时会重置日志等级的问题

## 2.0.7.12 — 2026-03-10

- 插件管理功能：
  - 在导航面板中添加插件管理菜单项,用于插件信息管理
  - 实现插件启用状态切换

- 设置功能增强：
  - 软件设置添加打开插件目录

- 提交参考：
- 40341b0c48cb081afa5959dcd64ac79f54503beb — feat(main): 添加插件管理页面功能
- 9f66a74c5658096b18be6f052e81f8faf3942370 — feat(settings): 添加打开插件目录功能

## 2.0.7.11 — 2026-02-09

- 更新服务增强：
  - 修复需要管理员权限才能操作的目录无法更新的问题
  - 增强安全删除目录功能，支持ACL权限修复和多尝试删除
  - 优化进程解锁逻辑，提升文件锁定检测和处理能力
  - 改进错误日志记录，添加详细的异常信息捕获和记录
  - 增强下载和解压过程中的状态更新和进度报告机制

- 包配置功能增强：
  - 在包配置页面添加全选/全不选按钮
  - 实现批量切换所有包可见性状态

- 提交参考：
- 3330515df35504a7b3f5df57ce55bdc937381573 — feat(package-config): 添加包配置可见性全选功能
- 65d08834341c35c238abf74b0f333e8ddab0f47c — feat(update): 实现下载URL覆盖和增强更新服务功能

## 2.0.7.10 — 2026-02-06

- 数据表格显示修复：
  - 修复数据表格无数据时列信息丢失的问题
  - 修复后续加载数据不显示列但能看到数据的问题

- 提交参考：
- e16068e2141ebdc717f392d3afb4d87ad0fe49de — fix: 修复数据表格无数据时列信息丢失的问题，修复后续加载数据不显示列但能看到数据的问题

## 2.0.7.9 — 2026-02-06

- 解除文件占用功能增强：
  - 增加操作面板和占用进程显示，支持控制终止和一键终止
  - 重新排列界面按钮顺序，将"解除占用"按钮移至前面

- 提交参考：
- 35cd301985d93fe19e1a6c46220c4be0aea87c72 — feat(unlock): 解除文件占用增加操作面板和占用进程显示，支持控制终止

## 2.0.7.8 — 2026-02-05

- Revit 破解工具集成：
  - 集成 Autodesk Universal Activation Tools 1.3.3.1 版本
  - 添加相关工具文件和说明文档

- 提交参考：
- 6f83b5e8e37fe967bd1f9bc5e9d18bc0689c0569 — feat(ui): 集成 Revit 破解工具

## 2.0.7.7 — 2026-02-04

- 产品可见性控制：
  - 添加可见性控制面板，在包配置页面中增加可见性设置
  - 更新主窗口加载逻辑，支持产品可见性过滤器
  - 修改筛选逻辑，支持按产品可见性进行分类显示

- 提交参考：
- 0b198a14f7b4ced64fe65e4252f3daaaf90ad8c0 — feat(services): 添加产品可见性控制功能

## 2.0.7.6 — 2026-01-29

- 看板统计：
  - 修复看板统计中挂起属于未开始的错误

- 提交参考：
- 39eb456699f834764a0ec3d4341c2bb91c5a69a6 — fix(pingcode): 修复状态判断逻辑并优化文档描述

## 2.0.7.5 — 2026-01-29

- PingCode评论功能：
  - 添加对回复评论内容、作者和ID的解析支持
  - 优化提及功能的交互，添加列表滚动定位
- 评论功能修复：
  - 修复工作项详情评论编辑输入@成员后再次输入@会出现两个@的问题
- 工作项详情：
  - 优化子工作项页签样式

- 提交参考：
- b11228cf4809842f753faf25d7ade7b7f6a87a5f — feat(pingcode): 添加PingCode评论回复功能支持
- 4ebacef8632039536291183866800aa9976c18ea — fix：修复工作项详情评论编辑输入@成员后再次输入@会出现两个@的问题
- 1650d2e3e6438e704943d6715cd4fdbc45071a4a — style：工作项详情子工作项页签样式优化

## 2.0.7.4 — 2026-01-22

- 工作项详情：
  - 支持子工作项查看页签与列表展示
  - 点击“子工作项”时懒加载列表，优化加载与交互
  - 无子工作项时隐藏页签，避免空页签
- 工作项详情评论：
  - 修复评论时间显示的本地时区问题
- 面包屑导航：
  - 当没有父工作项时，顶部仍支持跳转到 PingCode 工作项详情
- 迭代看板：
  - 不再设置父窗口，避免主界面被遮挡

- 提交参考：
- 692aaec3ede2c32563f0dfd1d368aea5c491963d — refactor：工作项详情子工作项加载优化
- 374973952644b6bf3b1c372c4cbd1886bceb9a81 — feat：工作项详情支持子工作项查看
- f1adadb219029aea86bb6817dd5deb368850ce3c — refactor：迭代看板不再设置父窗口，避免主界面被看板遮挡
- 7e9768d2823451ce1b5c5d404fdf3906ace695c2 — fix：修复工作项详情评论的时间时区不正确的问题
- 9bc61e92feb8c7d804ea2e5c9381eb8bd44457a6 — feat:无父工作项时工作项详情顶部也支持跳转到工作项的PingCode链接

## 2.0.7.3 — 2026-01-20

- 工作项详情评论：
  - 支持 @提及成员
  - 输入@自动弹出成员下拉；上下键选择与鼠标点击选择
  - 非选择操作（点击其他区域、滚动、失焦、Esc）自动关闭
  - 删除@时下拉立即消失；编辑框中展示“@+所选成员”
- 看板迭代筛选：
  - 排除已完成迭代，仅显示未完成迭代
- 筛选条件持久化：
  - “或/且”选择持久化保存
- 调试选项：
  - 支持 JSON 对象解析
- 其他：
  - 提示区域位置调整至右上角

- 提交参考：
- b6a7668ad14c378309d850d705baf017f17ff083 — refactor：删除或者esc等非选择人员的操作隐藏下拉成员框，提示交互体验
- 126206e188f3d2cab1ea4a16baf413bc5d0d1be3 — refactor：提示区域改为右上角
- 5c56b6d2dac7cb8024dc919e12b4db6682b93f3f — feat:工作项详情评论支持提及，输入@自动弹出成员选择
- b1d5fbb7afa85bd5948efbc32f752c439411ea69 — refactor:看板查看改为非已完成的迭代，不再是只能查看进行中的迭代
- 7056447a21018f90643a6ee522596c0fe0040484 — feat:调试选项的属性表支持解析json内的对象，扁平化叶子属性
- ebebd953c47cc61147c4df94d60ee8b98f3c694d — feat:筛选条件或与且的选择持久化保存




## 2.0.7.2 — 2026-01-19

- 筛选条件：
  - 支持“或/且”选择，增强筛选表达能力
- 主界面状态：
  - 取消索引回退，恢复仅按产品名匹配；旧状态文件若不含 ProductName 将不应用，以避免错配

- 提交参考：
- 054707f3d6a1cfeb7d1c1e798a2aff6b769dbc40 — feat：筛选条件支持或与且的选择
- a93a62bad9cc73511454d1444c899125bc59bec8 — refactor: 删除索引回退逻辑，避免错位；旧状态若无 ProductName 不应用

## 2.0.7.1 — 2026-01-19

- 工作项详情：
  - 开始/结束时间显示到分钟（本地时区）
- 工作项详情评论：
  - 并行上传图片，增量追加新评论，提高提交与刷新性能
- 内置包配置：
  - 修复 MaxiBIM（SH）的本地路径误指向 PMEP 的问题
- 主界面状态：
  - 保存主界面状态时加入 ProductName 键，避免新增内置包后本地路径错位

- 提交参考：
- 140f91e180d5e339a8ebb2e570958bcea7145781 — feat：工作项详情开始和结束时间支持具体时间
- 3e65a8bb17dbfb60add2462ba3368c2c789d81ab — fix：修复内置的SH的本地路径是PMEP的错误
- ea5a7d9cc338f0089d997b394dd3db5e91e3f7c4 — fix：保存主界面状态时增加产品名键值，避免增加内置包导致存储的本地包路径错位的问题

## 2.0.7.0 — 2026-01-16

- 工作项详情评论：
  - 添加评论支持
  - 支持添加图片
  - 交互与 PingCode 一致
  - 评论内容为空时加入我对 PingCode 评论功能的评价
  - 点击添加评论后跳转到工作项详情页面的最底部

- 产品管理：
  - 新的包管理保存后重新初始化可执行文件版本
  - 增加内置包MaxiBIM（SH）

- 看板：
  - 修复看板列统计故事点未格式化的问题
  - 修复“待完善”不属于进行中而属于未开始列的问题

- 提交参考：
- f6a04a696aaaf6f10eaddbeac806c50473b452d6 — feat：工作项详情支持添加评论
- 61d820a9034c72fed32654bf7494064f4ec1a34a — feat：工作项详情评论支持添加图片
- 713faab15753d1d9688050b4e386d7b9418c163e — refactor：工作项详情评论交互与PingCode一致
- 1d9ea22b74ec5c9b9a62c6d5bdf606697c7e2a9f — refactor：评论内容为空时加入我对PingCode评论功能的评价
- b6bb6b782a814350e246f140109fce7a0b0518f4 — feat：点击添加评论后跳转到工作项详情页面的最底部
- 6b1dec77fef7e675c311a18e796c7309d9647155 — feat：产品管理中增加新的包管理保存后重新初始化可执行文件版本
- 0ecd29e4f5595108b301089d8dbc5f398f672c42 — refactor：修复看板列统计故事点未格式化的问题
- 5a20c0992cf6594b1e24ae6edfa7f22bdef0a203 — fix(kanban): 修复待完善不属于进行中而属于未开始列的问题

## 2.0.6.2 — 2026-01-15

- 看板拖拽与刷新：
  - 优化拖拽工作项切换状态，完成拖拽切换后立即刷新
  - 引入自适应刷新策略：短期 5s 自动刷新，持续 30s 后恢复默认 20s
  - 修复改为全局滚动后的拖拽切换状态失效问题
- 看板布局：
  - 优化看板界面布局与操作体验
  - 移除底部的关闭按钮

- 提交参考：
- 547222699545b502a686fef7e558c13172542d8f — refactor: 优化拖拽状态切换，拖拽后立即刷新；刷新间隔 5s，持续 30s 后恢复 20s
- b3eaf156a830fd98e87dfa88380d3b14003b66b3 — refactor: 拖拽切换状态优化
- 4647a01a8dacab4aeb87f0076e7ca03b497a176d — feat: 增加工作项变化更新，修复全局滚动后拖拽切换失效
- 820b47a97eba33c2f64e904a2464c9380a25b916 — fix(kanban): 优化看板布局与体验；移除底部关闭按钮；调整刷新频率

## 2.0.6.1 — 2026-01-14

- 工作项详情页：
  - 添加工作项关注者功能支持
  - 实现关注者或参与者筛选

- 提交参考：
- 984c98595e20765a5d96e33d01c1618c02c8e583 — feat(workitem): 添加工作项关注者功能支持，筛选支持关注或者参与

## 2.0.6.0 — 2026-01-14

- 工作项详情页：
  - 添加参与者下拉选择
  - 实现基于参与者的筛选逻辑

- 日志功能：
  - 实现日志等级持久化保存

- 提交参考：
- d179d3a954bf9c736633684ea9890c0aa18f0ff8 — feat(work-item): 添加工作项看板参与者筛选功能
- de4f2a81f0b5761677993b0a2309bde4ebcb7234 — feat(logs): 日志等级持久化保存

## 2.0.5.7 — 2026-01-14

- 产品日志页：
  - 添加日志等级下拉选择框
  - 添加 ALL、ERROR、WARN、INFO 四个日志等级选项
  - 实现日志等级变更时批量更新所有包的 log4net.config 配置文件

- 成员统计：
  - 为各项统计数据添加格式化显示功能，包括未开始、进行中、已完成、已关闭等状态
  - 为优先级故事点添加格式化文本属性
  - 实现 FormatPoints 方法用于数值舍入和格式化显示

- 提交参考：
- e1be98805843e2a0785cc917a053576e0c5eff0b — feat(ProductLogs): 添加日志等级筛选功能
- b9b38d79b599dd7a15f41cc8f2fa61cc25fffa62 — feat(models): 更新成员统计项目模型以支持数值格式化显示

## 2.0.5.6 — 2026-01-13

- 工作项详情页：
  - 重构工作项详情窗口中的 WebView 初始化
  - 修复第一次打开工作项详情无法切换状态和更改故事点的问题

- 提交参考：
- 0d597abea1ab88d11b615d730f3cc750093a24b7 — refactor(views): 重构工作项详情窗口中的 WebView 初始化
- 39f4c40c1d6c751f84fd8b86a51e21bfb455fe9c — fix: 修复第一次打开工作项详情无法切换状态和更改故事点的问题

## 2.0.5.5 — 2026-01-13

- 工作项详情页：
  - 增加工作项详情页的状态选择
  - 增加工作项详情页的故事点编辑

- 提交参考：
- e0dcc0f7f45d467ef46febaa48b34068b77b07c6 — feat(workitem): 增加工作项详情页的状态选择和故事点编辑

## 2.0.5.4 — 2026-01-08

- 看板统计功能：
  - 添加了统计已关闭工作项
  - 调整了数据表格列的顺序，将已关闭状态列插入到已完成列之后
  - 修复了总计数计算，排除已关闭工作项的故事点

- 提交参考：
- 937198e9248f2f3aa11e99b9ce41116ca42d0781 — feat(stats): 添加已关闭状态统计功能


## 2.0.5.3 — 2026-01-07

- 产品日志页面功能：
  - 在产品日志页面添加打开产品目录按钮
  - 在Revit版本选择区域添加打开Revit目录按钮

- 提交参考：
- 40a666e634d82dd7d3b682e8fcd4477c6d19bcac — feat(ProductLogsPage): 添加打开目录功能


## 2.0.5.2 — 2026-01-07

- 工作项详情页面改进：
  - 添加工作项详情页面的面包屑导航功能，支持父级工作项跳转
  - 添加故事点汇总信息的显示
  - 点击顶部面包屑中的本工作项跳转到pingcode工作项详情

- 工作项评论修复：
  - 修复了评论显示中缺少作者名称的问题，现在同时显示作者和时间信息
  - 添加了外部链接的处理逻辑，支持在浏览器中打开链接

- 提交参考：
- bce5a3060d4b7335fd8b23da2fa0463d2cbfd1db — chore(version): 更新程序集版本号
- 7c8b4a4088cd35125405ce0797eaa596021822a3 — fix(workitem): 修复工作项详情页面评论中的作者显示和链接处理问题
- f5689ae43734f1d66f311a14f1a0fe25a26a5be8 — feat(workitem): 添加工作项详情页面的面包屑导航和父级信息展示


## 2.0.5.1 — 2026-01-06

- WebView2缓存：
  - 为WebView2添加本地缓存支持，确保WebView2在启动时使用配置的缓存环境

- 提交参考：
- a2e3e430e828ee03f00c677869c0d913f63f4946 — feat(workitem): 为WebView2添加本地缓存支持


## 2.0.5.0 — 2026-01-06

- 工单详情功能：
  - 显示工作项详细信息
  - 添加了工作项详情模板嵌入资源
  - 在模板文件中添加版本元数据标记
  - 添加模板版本检查和自动提取更新
  - 在工作项详情页面实现了描述和示意图的渲染
  - 添加了工作项评论中的图片直接渲染
  - 添加了访问令牌处理逻辑以支持私有资源访问

- 工作项状态管理：
  - 添加状态方案解析和目标状态确定功能
  - 调整刷新定时器间隔为20秒

- 性能优化：
  - 优化工作项详情页面加载
  - 调整布局为单列显示
  - 移除属性表格显示

- 其他变更：
  - 修复看板视图中优先级字段显示错误，将严重程度字段替换为优先级字段显示

- 提交参考：
- dedb2cba466c6bbae7e4bec6491f419a66803568 — chore(release): 更新程序集版本号
- 86781920642afdb60be30f21b1d0583711bc37b2 — perf(workitem): 优化工作项详情页面加载
- 344a4736d0177fdc16f2d8f1c68b36fea77b555e — feat(workitem): 添加工作项详情模板嵌入资源
- 7d50f7bd039b4bc366a3b1ce1aa38d52ac3110a1 — feat(workitem): 实现工单详情页面附件和图片显示功能
- 624c9f48532bf594746cfa728894e210730b2fc5 — feat(workitem): 添加工作项详情窗口功能
- 915b57657df7f2bb079fc412dd33b018df376f3b — feat(workitem): 添加工作项状态管理和拖拽功能
- 29a2362e27324eae78c3288e108bcfb8eb3ae472 — feat(workitem): 添加工作项卡片组件并优化看板界面
- 6b8b679486b01e4d28445ab9002f839d3c7ba4aa — feat(converters): 添加状态转换器和工作项看板功能
- 4d40b4da11172e6137200ce36b57570d3953742b — fix(kanban): 修复看板视图中优先级字段显示错误
- 0d7a1e2a6f2180554b0fe18e35dde8cd00e81a61 — feat(pingcode): 添加工单详情查看功能

## 2.0.4.2 — 2025-12-24

- Revit查找优化：
  - 增加通过注册表GUID匹配查找Revit应用程序位置

- 迭代看板功能（暂不开放）：
  - 实现了基于WebView2的迭代看板页面，包含看板HTML文件和样式
  - 添加了看板窗口的WebView2组件，支持通过Web消息传递数据
  - 实现了看板数据的序列化和传递功能，支持状态分类
  - 在看板统计页面添加打开看板按钮
  - 集成 WebView2 控件显示 HTML 看板界面
  - 添加按成员筛选看板功能和定时刷新机制

- 提交参考：
- 912656ec0304a65e74ff27b8c7b560a5fe453251 — feat(kanban): 添加迭代看板功能并优化Revit查找机制
- c180c9d09cabe432693db11e4257decc4fbb6871 — feat(kanban): 添加迭代看板功能
- 6cd7f42da070f054c73e97660ef497edef158478 — chore(build): 更新项目配置和依赖管理

## 2.0.4.1 — 2025-12-23

- 看板统计优先级：
  - 添加最高优先级数、最高优先级故事点、较高优先级数、较高优先级故事点、其他优先级数和其他优先级故事点字段
  - 添加优先级分类逻辑，支持最高、较高和其他优先级的识别
  - 扩展状态识别逻辑，支持更多进行中状态的关键词

- 看板统计界面优化：
  - 重新组织查询条件控件布局，提升界面紧凑性
  - 调整项目、迭代、人员选择器宽度与间距
  - 修改数据表格行定义，优化垂直空间利用
  - 在结果文本中增加故事点总数统计信息
  - 查询逻辑中累加总故事点数并显示在界面

- 看板统计性能优化：
  - 使用 Task.WhenAll 并行执行，提高初始化阶段的数据加载效率
  - 新增按负责人聚合的故事点拆分接口调用，减少重复的 API 请求次数，提升页面渲染速度

- 提交参考：
- 9f6b6b5163955d53ecd41cca47acdc07f7a61176 — chore(version): 更新程序集版本号
- 62df23de4271cc84de5b2f7a9938237615475f4e — feat(stats): 添加优先级统计到看板统计
- 32445224c20ad10552a5e274dd3cd73ad4cd43b5 — feat(kanban): 调整统计页面布局并优化数据展示
- 7a7467d97eb3384f8d9e4a5f21c5ca5fe995dcb5 — feat(stats): 并行加载项目成员和迭代数据; perf(stats): 优化成员故事点统计查询性能

## 2.0.4.0 — 2025-12-22

- 看板统计功能：
  - 新增 看板统计 页面，支持项目、人员、迭代筛选
  - 实现与 PingCode API 的数据对接，获取项目成员及故事点统计
  - 在导航面板和主窗口中增加看板统计入口
  - 添加 MemberStatsItem 模型用于展示成员故事点分布

- 提交参考：
- 1bc21e194a62184a9b76292ca65f68ea4d2453ab — feat(kanban): 添加看板统计页面及PingCode集成

## 2.0.3.1 — 2025-12-18

- 更新脚本编码修复：
  - 使用 Encoding.Default 写入更新脚本以避免乱码
  
- 用户配置迁移：
  - 将用户配置预设文件迁移至 AppData 目录

- 提交参考：
- 3bd3dd88f165df03b596c7b257789552f39efa72 — fix(update): 修复更新脚本编码问题并调整配置路径

## 2.0.3.0 — 2025-12-08

- 编译顺序更新功能：
  - 新增 编译顺序 窗口用于处理 .sln 文件的依赖更新
  - 实现拖拽或选择文件方式添加待处理的解决方案文件
  - 提供清空列表、开始处理和关闭窗口功能
  - 集成 Updater 类解析并更新项目间的依赖关系
  - 在界面中显示每个文件的处理状态和消息提示
  - 在 产品分类 中增加编译顺序按钮入口

- 常规解除占用失败时提权强杀：
  - 检查剩余进程是否已退出
  - 当存在未退出进程且当前非管理员权限时，尝试以管理员身份强制终止进程
  - 使用taskkill命令并设置超时时间来确保进程被正确终止

- 提交参考：
- 44b74f95df85edf6d2c24360d4543e0d461a727b — feat(SlnTool): 添加编译顺序更新功能
- 5ea0df017c5372a354afa075af0b2283ef12e62f — feat: 常规解除占用失败时提权强杀

## 2.0.2.6 — 2025-12-05

- 解除文件占用功能：
  - 新增解锁工具窗口，支持选择目录或文件进行占用检测
  - 实现拖拽添加目标文件或目录的功能
  - 集成进程锁定检测与自动关闭逻辑
  - 提供可视化界面展示处理状态和结果反馈
  - 在主页添加入口按钮以打开解锁窗口

- 提交参考：
- ba650b07541a9da35d47cf4a962485f02b36bd8f — feat(unlock): 添加解除文件占用功能

## 2.0.2.5 — 2025-12-05

- 包管理筛选的优化：
  - 实现可见行版本信息加载功能
  - 新增方法用于加载可见包版本
  - 修改主窗口初始化逻辑，替换原有的全量加载为可见行加载
  - 更新刷新按钮点击事件，优先加载可见行版本信息
  - 在窗口内容渲染完成后触发一次可见行版本加载

- 包属性变更事件优化：
  - 恢复属性变化监听中的状态文本更新逻辑

- 提交参考：
- 6aa0cb63043530694da5a88ec8effbc32f90e3c9 — feat(main): 实现可见行版本信息加载功能

## 2.0.2.4 — 2025-12-04

- 注册表访问修复：
  - 修复注册表访问方式以支持64位系统
  - 使用 RegistryKey.OpenBaseKey 确保在64位系统上正确访问注册表
  - 替换原有的 Registry.LocalMachine.OpenSubKey 方式

- 日志目录检查逻辑优化：
  - 删除重复的目录存在性验证代码
  - 简化日志页面初始化流程
  - 移除不必要的 Revit 日志目录设置逻辑

- 包属性变更事件优化：
  - 注释掉包属性变更事件中的状态更新逻辑
  - 暂时禁用最新选中包的自动更新机制
  - 为后续重构保留代码参考
  
- 缺陷修复：
  - 修复筛选情况下无法切换选中包的问题

提交参考：
- 905f355fe6e724fc8dd554b5b135762ddd7a77a3 — fix(registry): 修复注册表访问方式以支持64位系统
- 905f355fe6e724fc8dd554b5b135762ddd7a77a3 — chore(logs): 移除冗余的日志目录检查逻辑
- 905f355fe6e724fc8dd554b5b135762ddd7a77a3 — refactor(ui): 注释掉包属性变更事件中的状态更新逻辑

## 2.0.2.3 — 2025-12-04

- 产品日志页面优化：
  - 移除包管理页面中的冗余Revit日志按钮
  - 将产品日志的日志展示重构为标签页形式
  - 增加Revit可执行版本选择下拉框
  - 改进Revit日志目录定位逻辑

提交参考：
- 37d1872cc2ef788d22486a6efc6352169da0306b — feat(ui): 优化产品日志页面布局并增强Revit日志功能

## 2.0.2.2 — 2025-12-04

- 直接升级到最新版本功能：
  - 在 设置 界面增加"升级到最新"按钮支持静默升级到最新版本

- 界面布局样式优化：
  - 使用自定义按钮控件替换原生 Button 控件以统一界面风格
  - 隐藏已弃用的包配置 GroupBox 区域

- 缺陷修复：
  - 修复 产品日志 页面中 Revit 日志目录获取逻辑，直接从导航进入也能正确显示内容


提交参考：
- e8a0b3cf18a7a3baf103c8e36a656c69e65426ba — feat(app): 新增直接升级到最新版本功能

## 2.0.2.1 — 2025-12-04

- Revit日志查看功能：
  - 包管理页面操作区新增 按钮`Revit日志` 用于查看所选Revit版本下的Journal日志。
  - 新增 产品日志 对Revit日志目录的支持
  - 实现从产品页面跳转到日志查看器并传递Revit目录
  - 支持显示和刷新Revit Journals目录下的日志文件
  - 优化日志文件列表展示和排序方式，最新的显示在最上方
  - 提供返回主页和重新选择目录的功能


提交参考：
- fe95ce7dfad5606f1d855f6fe1d5858eb2bc722d — feat(logs): 新增Revit日志查看功能

## 2.0.2.0 — 2025-12-03

- 调试选项功能：
  - 新增 `DebugOptionsWindow` 用于编辑调试配置。
  - 加载与保存 `DebugSetting.json`，支持属性表查看与编辑。
  - 在包管理界面添加“调试选项”按钮。
  - 扩展 `PackageInfo` 以支持调试选项命令。
  - 将“模式切换”按钮更名为“调试选项”。
  - 使用 `CDangerButton` 替换定版按钮样式。


提交参考：
- 730604784df19c8f67795bc25e7330e8d93b0056 — feat(setting): 添加调试选项功能

## 2.0.1.0 — 2025-12-02

- 产品定版支持：
  - 更新多个产品的配置项以支持定版路径。
  - 修改 `编辑包配置` 以支持新增或编辑定版路径。
  - 在 `产品分类` 添加“定版”按钮并实现点击事件处理。
  - 优化保存配置逻辑，支持重载无需重启。

- 仅下载 ZIP 包：
  - 实现仅下载 ZIP 包的命令逻辑与状态管理。

- 导航与入口：
  - 新增定版下载功能入口。

提交参考：
- 8f79235d2c48141170a76652b4ee9d94f0e2324f — feat(package): 添加仅下载ZIP包功能
- c90b397c86bb60097d2057a2156007816e40ffd4 — feat(package): 添加产品定版功能支持

## 2.0.0.0 — 2025-12-01

- 交互与界面重构：
  - 新增 `CommonLinksPage` 与 `CommonLinksWindow`，实现常用网址导航页面。
  - 新增 `LogViewerPage`，支持按类型/日期/级别筛选与搜索日志。
  - 新增 `LocalPathSettingsPage`，支持产品本地路径分组设置。
  - 主窗口集成左侧导航面板与中央页面框架；增加 `ICentralPage` 接口用于页面间通信。
  - 优化数据表格列宽与显示效果。

- 更新日志页面：
  - 新增 `ChangelogPage` 展示 Markdown 格式的更新日志。
  - 新增 `OpenChangelogPageCommand` 命令用于打开更新日志页面。
  - 将 `CHANGELOG.md` 设置为内嵌资源以支持多路径加载。

- 自动更新摘要：
  - 改为异步加载版本更新摘要，避免 UI 阻塞。
  - 将 `UpdateSummary.md` 更名为 `UpdateSummary.txt` 并移出嵌入资源，改为从远程服务器读取。
  - `BuildUpdatePromptContent` 与 `LoadVersionSummaries` 改为异步实现。

- 左侧导航改进：
  - 应用启动时默认选中“产品分类”，统一初始高亮状态。
  - 导航失败不更改选中项（如产品日志目录缺失时保留原选中项）。
  - 页面状态策略：离开页面未保存修改不保留，统一采用显式“保存”持久化。
  - 包管理配置重构：新增页面 `Views/PackageConfigPage`，抽象 `IPackageEditorHost` 以复用编辑/删除逻辑，更新 `Models/PackageItem` 依赖；主窗口命令改为导航到新页面。
  - 主页导航：点击“产品分类”导航到 `PackagesHomePage`，行为一致化。
  - 稳定性与交互优化：产品日志路径存在性检查与提示增强，日志查看页加载行为更稳健。

- 缺陷修复：
  - 修复“更新”和“签名取消”功能的启用状态控制不正确的问题。

提交参考：
- ad69b7e8 — feat(update): 异步加载版本更新摘要，改用远程 `UpdateSummary.txt`
- 5d878137 — fix(PackageInfo): 修复更新和签名取消功能启用状态控制
- fdb745d3 — feat(ui): 重构导航与更新日志页面
- 711477f9 — feat(ui): 交互重构（链接、日志、路径、框架与接口）

## 1.0.14.0 — 2025-12-01

- 自动更新提示历史更新要点。


## 1.0.13.0 — 2025-11-28

- 主界面新增筛选功能，支持弹出式筛选条件编辑器。
- 筛选条件持久化：保存/恢复 `DataGrid` 各列的筛选状态。
- 版本排序优化：FTP 目录支持带日期前缀的命名（如 `2025.09.30_v1.5.2`）。
- 常用链接扩充：新增 210 与 215 站点入口。
- 配置预设增强：当当前配置不在列表时自动创建“当前配置”卡片并选中。

提交参考：
- 4277cfe7 — feat(main): 添加筛选与版本排序优化
- 44dcef6e — chore(version): 版本号更新至 1.0.13.0
- 09423eb8 — feat(config): 当前配置缺失时自动创建卡片

## 1.0.12.0 — 2025-11-28

- 更新与签名加密流程支持取消操作（`CancellationToken` 全流程控制）。
- 外部工具运行支持取消与进程终止处理，增强稳定性。
- 新增 PowerShell 脚本：图标生成与程序签名自动化。
- 主窗口常用链接新增“传360”。
- 应用程序图标支持：新增 `App.ico` 并在项目中配置使用。
- 应用配置文件更新。

提交参考：
- f1efbda0 — feat(ui): 取消功能与脚本、链接、版本号至 1.0.12.0
- 88199b17 — feat(app): 更新应用程序配置文件
- 43bafccb — feat(ui): 添加应用程序图标支持

## 1.0.11.0 — 2025-11-28

- 主界面新增常用网址导航与“产品日志”查看功能。
- 集成 LogViewPro、VSCode、记事本多种日志打开方式；内置 LogViewPro。

提交参考：
- 74be57d2 — feat(ui): 常用网址导航与产品日志
- b998646b — chore(version): 版本号至 1.0.11.0

## 1.0.10.0 — 2025-11-27

- 配置预设界面重构：改为粘贴完整 INI 文本；新增 `RawIniContent` 存储原始文本。
- 配置卡片展示优化：高度自适应内容行数；支持按完整文本匹配。
- 新增日志详情查看窗口与双击查看详情功能；引入 `PresetToIniConverter`。
- 工程重构：按功能模块重组到 `Function` 目录；优化命名空间与引用路径；统一 `System.IO.Path`。

提交参考：
- c21ddf81 — feat(config): 预设界面与逻辑重构，版本至 1.0.10.0
- 718678c1 — refactor(project): 文件结构与命名空间优化

## 1.0.9.0 — 2025-11-27

- 本地路径设置支持“按版本配置与获取”。
- 界面展示优化、列索引调整、状态判断修复；更新包服务路径引用逻辑。

提交参考：
- 3a9f93e3 — feat(package): 版本本地路径与界面优化，版本至 1.0.9.0

## 1.0.8.0 — 2025-11-26

- 新增 DNS 设置窗口与编辑对话框：支持添加/编辑/删除 IPv4 DNS。
- 集成系统网络设置跳转；主窗口按产品控制 DNS 功能可见性。

提交参考：
- c9c16b83 — feat(dns): DNS 设置与编辑功能
- 88aaa4bc — chore(version): 版本号至 1.0.8.0

## 1.0.6.0 — 2025-11-25

- 新增“解锁更新”功能：检测并关闭占用进程后继续更新。
- 引入 Windows Restart Manager API：更精确的进程锁定检测。
- 优化更新流程：安全解压、暂存占用文件、完善进度与日志。

提交参考：
- ef3652f0 — feat(package): 解锁更新与流程优化，版本至 1.0.6.0

## 1.0.5.0 — 2025-11-25

- 主窗口布局重构：自定义导航控件、按钮区、搜索框与分栏布局。
- 支持动态分割器调整宽度；统一控件样式（文本块、组合框、进度条）。
- 路径有效性检查逻辑优化：区分路径为空与目录不存在。

提交参考：
- 8dd2bc86 — feat(ui): 主窗口布局与控件样式重构，版本至 1.0.5.0

## 1.0.4.0 — 2025-11-18

- 配置预设管理：新增 `ConfigPreset` 与 `ConfigPresetStore`，支持内置与自定义预设。
- 新增 `ConfigPresetWindow` 与 `AddPresetWindow`；主窗口增加“切换配置”与相关命令。
- 应用自动更新：新增 `AppUpdateService`，支持检查与下载，版本比对与后台替换。
- 进度与状态修复：嵌入工具状态信息失效问题修复；新增 `DebugSettingsService` 与 `EmbeddedToolRunnerService`。

提交参考：
- 9612bc49 — feat(config): 预设配置管理，版本至 1.0.4.0
- 36ce4466 — feat(update): 自动更新功能（配置来源与比对优化）
- 66c53dcc — fix(package): 进度与状态信息修复，版本至 1.0.4.0

## 1.0.3.0 — 2025-11-17

- 日志查看器：支持按类型/日期/级别筛选、搜索与自动滚动；添加打开日志按钮。
- 日志服务与管理员权限：统一日志记录，应用启动时检测并请求管理员权限。
- 自动更新功能首次引入：检查下载与版本比对、替换脚本、错误处理与提示。

提交参考：
- e626464b — feat(log): 日志查看器与解压逻辑优化，版本至 1.0.3
- 870059dd — feat(app): 日志服务与管理员权限自动提升
- ee2ef547 — feat(update): 自动更新功能，版本至 1.0.3.0

## 1.0.2 — 2025-11-12

- 默认阶段初始化：避免仅有“下载”阶段时总进度异常跑满。
- 设置窗口版本号更新为 1.0.2。

提交参考：
- f4b2d7dc — feat(package): 默认阶段初始化
- 2b511b7a — chore(settings): 应用版本号更新至 1.0.2

## 1.0.1 — 2025-11-10

- 本地包路径设置：新增 `LocalPathSettingsWindow` 与 `LocalPathInfo` 展示。
- 调试模式切换：新增 `IsDebugMode` 持久化与 UI 按钮。
- FTP 目录版本号排序与配置操作按钮启用状态控制。
- 设置项：新增程序入口文件是否带 `G` 的选项（`ProgramEntryWithG`）。

提交参考：
- 50151ff0 — feat(settings): 本地包路径设置
- 1e7ec6e9 — feat(package): 调试模式切换
- 46c9067c — feat(ftp): 目录版本排序与按钮启用控制
- f51f4721 — feat(settings): 程序入口文件是否带 G 的配置

## 1.0.0 — 2025-10-31

- 依赖嵌入：集成 Costura.Fody 与 Fody 配置，实现依赖程序集嵌入。
- 应用程序设置与数据管理：主界面状态保存/加载，缓存/状态/数据清除，设置序列化与反序列化。
- 服务能力：应用程序查找（多策略）、数据持久化；FTP 文件列表与时间解析与排序。
- 初始主界面：包管理器主界面基础搭建。

提交参考：
- 4283a5e1 — feat(project): 集成 Costura.Fody
- c130971d — feat(settings): 应用程序设置与数据管理
- 6468c9d9 — feat(services): 应用查找与数据持久化、FTP 文件解析
- ea46a3fa — feat: 包管理器主界面
