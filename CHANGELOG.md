...
- 代码工作区刷新提速：
  - 子仓库状态采集改为并行并加全局 8 并发限流；子仓库目录发现合并为单趟遍历并缓存 1 分钟，消除刷新时的进程风暴。
  - 状态应用改为差分更新：变更文件内容未变时保留原集合，子仓库布局一致时原地更新字段，轮询不再整表重绑。
  - 仓库卡片 VCS 摘要增加脏检查，输入未变化时跳过近 30 项派生属性重算。
  - 差异查看器等宽测量按字符缓存宽度，排版次数从按行数降到按不同字符数。

- 密语离线信箱投递与统一同事列表：
  - 对方离线时密语改走信箱：端到端加密信封投递，消息标记「已投递」，对方上线拉取后送达并回执「已拉取」。
  - 信箱客户端维护公钥目录与按设备分隔的密文信箱，拉取即删，超 24 小时的信封自动过期清理。
  - 已读/销毁回执直连不可达时改投对方信箱，文件名按回执种类分离互不覆盖；已读回执携带对方自毁时长，重连后按剩余时间同步倒计时。
  - 新增本地联系人与会话存储：未销毁消息整体 DPAPI 加密落盘，重启后恢复未读与投递状态，退出时自动保存（防抖修复为必然落盘）。
  - 同事列表合并在线设备与离线联系人：徽标区分在线/离线与计数摘要，状态翻转保留选中并即时重排，离线条目仅支持密语。
  - 信箱轮询频率接入设置（30/60/120 秒、0 关闭、默认 60），有窗口打开或存在未读时加速到 20 秒；打开传输页或密语窗口时主动拉取一次。
  - 发布工具的更新清理改为只删除更新说明文件本身，保留服务器上的密语信箱目录，并同步重编译二进制。

- 密语已读判定与界面重构：
  - 已读判定改为视口触发：消息气泡进入视口并停留约 1 秒才标记已读，打开窗口自动定位到第一条未读。
  - 失焦或关窗时立即焚毁已读消息并通知对端，未读消息保留至真正阅读；防截屏与失焦遮蔽（不透明）保留。
  - 聊天界面重做：气泡内嵌引信进度条、未读高亮与销毁灰烬占位，头部改为头像+截图保护徽章布局，消息区隐藏滚动条。
  - 状态文本按方向区分：发送方显示送达/已拉取/已读回执，接收方显示未读与阅读计时。
  - 自毁时长接入应用设置，支持 5/10/30 秒，非法值回退 5 秒。
  - 新增密语自测线路（受限功能）：单机模拟成对会话验证收发、已读与焚毁链路，入口默认隐藏。

- 密语离线投递可靠性修复：
  - 密语直连新增 2 秒连接限时，失败或超时自动降级信箱投递，消息不再被直接销毁；
  - 对端已收到直连消息时，信箱副本按消息号幂等去重，不会重复显示。
  - 密钥对改为 DPAPI 加密落盘、跨重启复用，避免重新生成后历史信箱信封与已发布公钥无法解密验证。
  - 信箱读取改用写账号凭据，修复默认读账号被服务器停用（530 未登录）导致拉取失败。
  - 对方公钥优先从信箱公钥目录获取（对方最近启动发布，最新鲜），目录不可达时回退本地缓存。

- 密语信箱拉取提速：
  - 并发调用共享同一次在途拉取任务，页面与窗口同时触发不再空手而归。
  - 请求统一 10 秒超时并复用控制连接，网络抖动不再拖垮启动链路。
  - 启动时拉取提前到目录建设与公钥发布之前；信封并行下载后即删，离线消息更早可见。
