...
- 单条长度要克制但不机械限短：优先保持单行可读，常见条目以约 45-65 个中文字符为宜，确需承载关键行为与影响时可到 70，但不作为硬下限——更短只要信息完整同样合格；如果两条短句合并后更清楚，应合并成一条信息完整的 bullet。只有视觉上接近终端换行边界、信息密度过高或实现细节堆叠时，才继续压缩。
...
- 单条正文纯长度超限（按中文字符计，不含代码标识符本身长度）：超过 70 个字符每条 -6，超过 90 个字符每条 -10。此为纯长度规则，无论内容是否紧凑都会触发；代码标识符数量已由上方"≥2 个代码标识符"条目另行扣分，不在此叠加。
