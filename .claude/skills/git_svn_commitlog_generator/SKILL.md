...
- 正文建议 2–5 条 `- ` 条目，复杂或文件较多时最多 8 条；条目数不得超过该提交组 `ItemsIncludedDefaultLog` 文件数。若 `ItemsExcluded` 非空，附 `本次排除清单`（`#Id Path`）。
...
5. **紧凑性/单行性**：每条是否优先单行？是否能靠删除实现细节变短？是否有多余续行、孤立短尾行，或在不该换行的地方换了行？如有，必须合并回单行或按语义重写。
6. **scope 归源**：标题和正文里出现的每个项目/模块名，是否都能在 `ItemsIncludedDefaultLog` 里找到来源？
7. **主题聚合**：同一功能链路跨多个文件时是否已合并？是否为了覆盖文件而按文件流水账？若是，必须合并压缩。
8. **细节克制**：函数名、属性名、字段名、JSON 字段名、脚本名是否被过度展开？若实现细节不是理解变更影响所必需，必须删掉或并入主题。
...
以下情况不得直接判定通过：同一 `run-commit-push-choice.ps1` 相关变更拆成多条重复条目；条目围绕文件名流水账；把“新增函数 / 新增属性 / 新增字段 / 新增 JSON 字段 / 更新文档”拆成低信息量条目；条目偏长但没有合并主题或压缩表达。条目数本身不是问题，话太多、实现细节过多、按文件流水账才是问题。
