...
1. `全部加入`：将 `NeedsAdd[]` 中所有候选加入版本管理
2. `选择部分`：展示可加入编号示例（如 `1,3,8` 或 `2-6`）
3. `不加入`：不加入任何未跟踪文件（等价 `none`）
...
- MftEnumerator 新增 _childDirectoryFrnsByParent 父子目录映射与 TryGetDirectorySubtree 路径解析，将路径前缀直接定位到目录子树 FRN 集合
- MemoryIndex 新增 ParentSortedArray 按父目录排序数组及 GetSubtreeCandidates 二分查找，高效提取子树候选集
- IndexService 搜索路径前缀时优先使用目录子树前置过滤，解析失败时回退到原有后置过滤，并输出 PATH PREFILTER 诊断日志
- SharedIndexMemoryProtocol 移除多余 Flush()、用 ThreadStatic 缓冲区优化字符串序列化、WriteResponse/WriteRequest 返回字节数
- SharedIndexServiceClient 将 WaitForResponseAsync 从阻塞式 Task.Run+WaitHandle 改为异步 RegisterWaitForSingleObject+TaskCompletionSource
...
1. 提交日志（始终只输出一条）\n
2. 正文条目使用 `- ` 短横线，不使用编号\n
3. `本次排除清单`：列出 `#Id Path`（来自 `ItemsExcluded`）\n
