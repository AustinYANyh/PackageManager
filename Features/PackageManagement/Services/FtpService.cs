...
namespace PackageManager.Services
{
    ...
    public class FtpService
    {
        ...
        public FtpService(DataPersistenceService dataService)
        {
        ...
        private static void ResolveDefaultCredentials(ref string username, ref string password)
        {
            ...
            var cred = ServiceLocator.Resolve<CredentialStore>()?.GetFtpReadCredential();
            username = cred?.UserName ?? "hwclient";
            password = cred?.Password ?? "hw_ftpa206";
        }
        ...
        public List<string> GetDirectories(string serverUrl, string username = null, string password = null)
        {
            ...
            var filterLog = _dataService?.LoadSettings()?.FilterLogDirectories ?? true;
            if (filterLog)
            {
