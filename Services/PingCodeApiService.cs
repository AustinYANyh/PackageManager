    ...
    public class PingCodeApiService
    {
        ...
        private async Task<string> BuildAttachmentsHtmlAsync(JToken v)
        {
            try
            {
                ...
                foreach (var a in arr)
                {
                    ...
                    var extImg = nameLower.EndsWith(".png") || nameLower.EndsWith(".jpg") || nameLower.EndsWith(".jpeg") || nameLower.EndsWith(".gif") || nameLower.EndsWith(".bmp") || nameLower.EndsWith(".webp") || nameLower.EndsWith(".svg") || nameLower.EndsWith(".tif") || nameLower.EndsWith(".tiff") || nameLower.EndsWith(".avif");
        ...
        private string AppendAccessTokenIfNeeded(string url)
        {
            try
            {
                ...
                var tk = _token;
                if (string.IsNullOrWhiteSpace(tk)) return u;
                if (u.Contains("?")) return $"{u}&access_token={Uri.EscapeDataString(tk)}";
